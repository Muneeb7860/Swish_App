...
const B2bDashboard = () => {
	...
	const connect = useCallback(() => {
		...
		const ws = new WebSocket(
			"ws://localhost:8080/ws/notifications/b2b?userId=b2b-customer-123&access_token=mock_token_for_now",
		);
		...
		ws.onmessage = (event) => {
			...
			try {
				...
				if (payload.type === "WELCOME") {
					...
					setOrderStatus(
						payload.payload?.ai_status || payload.ai_status || "UNKNOWN",
					);
		...
		ws.onclose = (event) => {
			...
			if (reconnectAttemptRef.current < MAX_RECONNECT_ATTEMPTS) {
				const delay = Math.min(
					1000 * 2 ** reconnectAttemptRef.current + Math.random() * 500,
					30000,
				);
				console.log(
					`Reconnecting in ${Math.round(delay)}ms (attempt ${reconnectAttemptRef.current + 1}/${MAX_RECONNECT_ATTEMPTS})`,
				);
	...
	return (
		...
		>
			...
			>
				...
				<div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
					<span
						...
						title={`${notifications.length} unread`}
					>
						🔔{" "}
			...
			<p>
				Place wholesale orders and monitor AI credit evaluation in real-time.
			</p>
			...
			>
				...
				{orderStatus === "PENDING" ? (
					...
					<div
						style={{ padding: "15px", textAlign: "center", color: "#718096" }}
					>
					...
					>
						...
						{orderStatus === "HUMAN_TRIAGE" && (
							<p style={{ color: "#c53030", marginTop: "10px" }}>
								⚠️ Alert: Order exceeds $1M threshold. Routed to human
								underwriter queue.
							</p>
						)}
					</div>
