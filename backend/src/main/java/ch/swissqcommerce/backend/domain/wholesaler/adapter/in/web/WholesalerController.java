...
public class WholesalerController {
    ...
    @Autowired private WholesalerUseCase wholesalerService;
    ...
    @GetMapping("/restocks")
    public ResponseEntity<List<B2BRestockOrder>> getAssignedRestocks(
            @RequestParam(required = false, defaultValue = "WHOLESALER-1") String id) {
    ...
    public ResponseEntity<B2BRestockOrder> createRestockOrder(
        ...
        B2BRestockOrder order =
                wholesalerService.createRestockOrder(
    ...
    @GetMapping("/invoices")
    public ResponseEntity<List<B2BInvoiceDto>> getInvoices(
            @RequestParam(required = false, defaultValue = "WHOLESALER-1") String id) {
        List<B2BRestockOrder> restocks = wholesalerService.getAssignedRestocks(id);
        List<B2BInvoiceDto> invoices =
                restocks.stream()
                        .map(
                                order ->
                                        new B2BInvoiceDto(
                                                ...
                                                order.getWholesaler() != null
                                                        ? order.getWholesaler().getWholesalerId()
                                                        : id,
    ...
    public ResponseEntity<PurchaseOrder> generateReplenishmentOrders(
            @Valid @RequestBody GeneratePoRequest request) {
        PurchaseOrder po =
                wholesalerService.generateReplenishmentOrders(
                        request.getStoreId(), request.getVendorName(), request.getRequestedItems());
        return ResponseEntity.status(201).body(po);
    }

    @PostMapping("/po/{id}/receive")
    public ResponseEntity<PurchaseOrder> receiveGoods(
            @PathVariable String id, @RequestBody ReceiveGoodsRequest request) {
        PurchaseOrder po =
                wholesalerService.receiveGoods(
                        id, request.getItemReceipts(), request.getGrnFileUrl());
