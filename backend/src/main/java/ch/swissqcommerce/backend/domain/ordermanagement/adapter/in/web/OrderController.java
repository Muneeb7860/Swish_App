...
public class OrderController {
    ...
    /**
     * Asserts the authenticated principal owns {@code customerId}, or is an ADMIN.
     * Mirrors the ownership guard used across the customer/payment domains so a
     * caller cannot open an order saga on another customer's behalf (IDOR).
     */
    private void assertOwnership(String customerId) {
        ...
        if (!isAdmin && !customerId.equalsIgnoreCase(auth.getName())) {
            throw new AccessDeniedException("Access denied: you may only place orders for your own account.");
        }
    }

    @PostMapping
    public ResponseEntity<?> placeOrder(@RequestParam String orderId, @RequestParam String customerId) {
