...
public class ResilientLlmGateway implements LlmGatewayPort {
    ...
    private LlmResponse executeCallChain(String prompt) {
        ...
        if (geminiFreeAdapter.isConfigured()) {
            if (!isPromptPiiFree) {
                log.warn("Governance degraded and PII detected — blocking Gemini cloud LLM; "
                        + "returning governance-degraded response for HITL escalation.");
                return governanceDegraded();
            }
            log.info("Governance degraded — routing PII-free prompt to gated Gemini cloud fallback.");
        ...
        if (kimiLlmAdapter.isConfigured()) {
            if (!isPromptPiiFree) {
                log.warn("Governance degraded and PII detected — blocking Kimi cloud LLM; "
                        + "returning governance-degraded response for HITL escalation.");
                return governanceDegraded();
            }
            ...
            try {
                ...
                log.warn("Kimi Cloud fallback failed ({}). Dropping to local mock.", e.getMessage());
            }
        }
