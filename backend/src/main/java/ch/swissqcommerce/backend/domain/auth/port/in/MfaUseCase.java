 ...
 *
 * <p>Follows the hexagonal boundary (ADR-001): the controller calls this port; the concrete service
 * ({@link ch.swissqcommerce.backend.domain.auth.core.service.MfaService}) lives in core.
 */
public interface MfaUseCase {

    /**
     * Generate a 6-digit OTP, store it in Redis keyed to the session token, and return the session
     * token. The caller (controller) is responsible for delivering the OTP to the user out-of-band
     * (email/SMS).
     *
