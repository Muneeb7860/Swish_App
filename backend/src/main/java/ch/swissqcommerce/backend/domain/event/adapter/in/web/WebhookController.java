 ...
 *
 * <p>Accepts callback payloads from n8n workflows and routes them into the
 * event pipeline via {@link EventUseCase}. Authentication is handled via
 * HMAC-SHA256 signature verification using a shared secret, since webhook
 * callbacks originate from automation engines — not user sessions.</p>
 *
 * <p>The webhook endpoint is added to the Spring Security {@code permitAll}
 * list because it uses its own HMAC authentication scheme independent of JWT.</p>
 */
@RestController
...
public class WebhookController {
    ...
    public WebhookController(
        ...
        this.webhookReceived = Counter.builder("webhook_events_received_total")
                .description("Total webhook events received")
                .register(meterRegistry);
        this.webhookRejected = Counter.builder("webhook_events_rejected_total")
    ...
    public ResponseEntity<Map<String, String>> receiveN8nWebhook(
        ...
        if (signature == null || signature.isBlank()) {
            ...
            log.warn("WebhookController: Rejected n8n webhook — missing X-Webhook-Signature header.");
        ...
        try {
            ...
            String payloadJson = payloadNode.isMissingNode()
                    ? "{}"
            ...
            log.info("WebhookController: Accepted n8n webhook — eventType={}, aggregateId={}, savedEventId={}",
                    eventType, aggregateId, savedEvent.getEventId());
            ...
            log.error("WebhookController: Failed to process n8n webhook payload: {}", e.getMessage(), e);
            return ResponseEntity.badRequest()
