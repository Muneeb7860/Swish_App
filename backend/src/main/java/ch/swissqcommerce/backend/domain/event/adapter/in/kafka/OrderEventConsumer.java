 ...
 *
 * <p>Handles order lifecycle events (placed, delivered, cancelled) published
 * by the Outbox Scheduler and closes the event loop by logging, correlating,
 * and triggering downstream actions (e.g. notification dispatch).</p>
 */
@Component
public class OrderEventConsumer {
    ...
    public OrderEventConsumer(ObjectMapper objectMapper, MeterRegistry meterRegistry) {
        ...
        this.orderEventsProcessed = Counter.builder("kafka_consumer_events_processed_total")
                .tag("topic", "order.events")
                .description("Total order events successfully processed")
                .register(meterRegistry);
        this.orderEventsFailures = Counter.builder("kafka_consumer_processing_errors_total")
                .tag("topic", "order.events")
    ...
    public void consume(
        ...
        try {
            ...
            log.info("OrderEventConsumer: Received event [type={}, aggregateId={}, correlationId={}]",
            ...
            log.info("OrderEventConsumer: Successfully processed event [type={}, aggregateId={}]",
                    eventType, aggregateId);
        } catch (Exception e) {
            orderEventsFailures.increment();
            log.error("OrderEventConsumer: Failed to process event [type={}, aggregateId={}, correlationId={}]: {}",
    ...
    private void processOrderEvent(String eventType, String aggregateId, JsonNode eventData) {
        switch (eventType) {
            case "order.placed":
                log.info("OrderEventConsumer: Order PLACED — aggregateId={}, data={}", aggregateId, eventData);
                // Future: trigger inventory reservation, notification dispatch
                break;
            case "order.delivered":
                log.info("OrderEventConsumer: Order DELIVERED — aggregateId={}, data={}", aggregateId, eventData);
                // Future: trigger delivery confirmation notification
                break;
            case "order.cancelled":
                log.info("OrderEventConsumer: Order CANCELLED — aggregateId={}, data={}", aggregateId, eventData);
                // Future: trigger refund workflow, inventory release
                break;
            default:
                log.warn("OrderEventConsumer: Unknown order event type '{}' — skipping.", eventType);
        }
    }
}
