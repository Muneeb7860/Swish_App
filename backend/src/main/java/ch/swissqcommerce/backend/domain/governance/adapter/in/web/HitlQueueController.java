...
/**
 * Supervisor HITL console API (Phase 8). Serves the unified queue (B2B procurement overrides +
 * agent escalations) and routes approve/reject by the item's composite id. ADMIN-only and
 * idempotent (resolving a non-pending item yields 409 via TicketAlreadyResolvedException).
 */
@RestController
...
public class HitlQueueController {
    ...
    public ResponseEntity<Map<String, Object>> approve(
            @PathVariable String id, @RequestBody OverrideRequest request) {
    ...
    public ResponseEntity<Map<String, Object>> adjust(
            @PathVariable String id, @RequestBody AdjustRequest request) {
        governanceUseCase.adjustHitlItem(
                id, request.getNewPrice(), request.getOperator(), request.getReason());
        return ResponseEntity.ok(
                Map.of(
                        "id", id,
                        "status", "ADJUSTED",
