 ...
 *
 * <p>Handles payment lifecycle events (authorized, captured, failed, refunded) published by the
 * Outbox Scheduler. Correlates payment state transitions with their parent orders via the aggregate
 * ID.
 */
@Component
public class PaymentEventConsumer {
    ...
    public void consume(
        ...
        try {
            ...
            log.info(
                    "PaymentEventConsumer: Received event [type={}, aggregateId={},"
                            + " correlationId={}]",
                    eventType,
                    aggregateId,
                    correlationId);
            ...
            log.info(
                    "PaymentEventConsumer: Successfully processed event [type={}, aggregateId={}]",
                    eventType,
                    aggregateId);
        } catch (Exception e) {
            paymentEventsFailures.increment();
            log.error(
                    "PaymentEventConsumer: Failed to process event [type={}, aggregateId={},"
                            + " correlationId={}]: {}",
                    eventType,
                    aggregateId,
                    correlationId,
    ...
    private void processPaymentEvent(String eventType, String aggregateId, JsonNode eventData) {
        switch (eventType) {
            case "payment.authorized":
                log.info(
                        "PaymentEventConsumer: Payment AUTHORIZED — aggregateId={}, amount={}",
                        aggregateId,
                        eventData.path("amount"));
                // Future: update order status to PAYMENT_AUTHORIZED
                break;
            case "payment.captured":
                log.info(
                        "PaymentEventConsumer: Payment CAPTURED — aggregateId={}, amount={}",
                        aggregateId,
                        eventData.path("amount"));
            ...
            case "payment.failed":
                log.warn(
                        "PaymentEventConsumer: Payment FAILED — aggregateId={}, reason={}",
                        aggregateId,
                        eventData.path("reason"));
                // Future: cancel order, notify customer, trigger retry/HITL
                break;
            case "payment.refunded":
                log.info(
                        "PaymentEventConsumer: Payment REFUNDED — aggregateId={}, amount={}",
                        aggregateId,
                        eventData.path("amount"));
            ...
            default:
                log.warn(
                        "PaymentEventConsumer: Unknown payment event type '{}' — skipping.",
                        eventType);
        }
    }
}
