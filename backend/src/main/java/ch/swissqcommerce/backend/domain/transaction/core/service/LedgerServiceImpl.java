...
public class LedgerServiceImpl implements LedgerUseCase {
    ...
    /**
     * Records a balanced double-entry transaction in the system. Enforces SERIALIZABLE isolation to
     * avoid concurrent race conditions on wallet balances.
     */
    public JournalEntry recordTransaction(
            String reference, String description, List<LedgerLeg> legs) {
        ...
        if (totalDebit.compareTo(totalCredit) != 0) {
            throw new IllegalArgumentException(
                    String.format(
                            "Unbalanced transaction. Total Debits: %s, Total Credits: %s",
                            totalDebit, totalCredit));
        }

        // 2. Fetch the hash of the last entry for chaining
        Optional<JournalEntryEntity> lastEntryOpt =
                journalEntryRepository.findFirstByOrderByEntryIdDesc();
        String prevHash =
                lastEntryOpt
        ...
        for (LedgerLeg leg : legs) {
            LedgerLineEntity lineEntity =
                    LedgerLineEntity.builder()
    ...
    private void adjustActorWallet(
            String accountType, String actorId, BigDecimal debit, BigDecimal credit) {
        ...
        BigDecimal netChange =
                credit.subtract(debit); // Credit increases wallet, debit decreases it
    ...
    public List<LedgerLine> getCustomerLedger(String customerId) {
        return ledgerLineRepository
                .findByAccountTypeAndActorIdOrderByLineIdDesc("customer", customerId)
                .stream()
                .map(
                        entity ->
                                LedgerLine.builder()
