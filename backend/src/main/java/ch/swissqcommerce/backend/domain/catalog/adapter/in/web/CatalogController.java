...
public class CatalogController {
     ...
     * defense-in-depth: it enforces the role at the method layer even if the
     * gateway/OPA URL-level policy is misconfigured or disabled (it currently
     * fails closed for {@code /api/v1/**}, but this must not be the only control).
     */
    ...
    public ResponseEntity<?> createProduct(@RequestBody ProductListing listing) {
        ...
        if (listing.getBasePrice() == null || listing.getBasePrice().compareTo(BigDecimal.ZERO) < 0) {
            return ResponseEntity.badRequest().body(Map.of("error", "basePrice must be present and non-negative."));
        }
