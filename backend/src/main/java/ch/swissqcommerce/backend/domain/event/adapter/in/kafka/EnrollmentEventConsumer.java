 ...
 *
 * <p>Handles rider and merchant onboarding state transitions published
 * by the Outbox Scheduler. Enables reactive workflow triggers when
 * enrollment milestones are reached (e.g. L1→L2→L3 validation).</p>
 */
@Component
public class EnrollmentEventConsumer {
    ...
    public EnrollmentEventConsumer(ObjectMapper objectMapper, MeterRegistry meterRegistry) {
        ...
        this.enrollmentEventsProcessed = Counter.builder("kafka_consumer_events_processed_total")
                .tag("topic", "enrollment.state_change")
                .description("Total enrollment events successfully processed")
                .register(meterRegistry);
        this.enrollmentEventsFailures = Counter.builder("kafka_consumer_processing_errors_total")
                .tag("topic", "enrollment.state_change")
                .description("Total enrollment event processing failures")
                .register(meterRegistry);
    }

    @KafkaListener(topics = "enrollment.state_change", groupId = "${spring.kafka.consumer.group-id}")
    public void consume(
        ...
        try {
            ...
            log.info("EnrollmentEventConsumer: Received event [type={}, aggregateId={}, correlationId={}]",
            ...
            log.info("EnrollmentEventConsumer: Successfully processed event [type={}, aggregateId={}]",
                    eventType, aggregateId);
        } catch (Exception e) {
            enrollmentEventsFailures.increment();
            log.error("EnrollmentEventConsumer: Failed to process event [type={}, aggregateId={}, correlationId={}]: {}",
    ...
    private void processEnrollmentEvent(String eventType, String aggregateId, JsonNode eventData) {
        ...
        log.info("EnrollmentEventConsumer: State change — type={}, aggregateId={}, enrollmentType={}, newState={}",
                eventType, aggregateId, enrollmentType, newState);
