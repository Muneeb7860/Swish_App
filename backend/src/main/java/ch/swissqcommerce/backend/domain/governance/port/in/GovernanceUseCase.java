...
public interface GovernanceUseCase {
    ...
    /**
     * Unified supervisor queue (Phase 8): pending B2B procurement overrides + pending agent
     * escalations, as one list of {@link HitlItem}.
     */
    List<HitlItem> getPendingHitlItems();

    /**
     * Approve or reject a unified HITL item by its composite id ({@code "PA-<id>"} for procurement,
     * {@code "AQ-<ticketId>"} for agent escalations). Idempotent: resolving an already-resolved
     * item throws {@code TicketAlreadyResolvedException}.
     */
