...
public class LogisticsPolicy {
    ...
    public PolicyDecision evaluate(AgentSuggestionEntity s) {
        try {
            ...
            double costDelta =
                    rec.has("shipping_cost_delta")
                            ? rec.get("shipping_cost_delta").asDouble()
                            : 0.0;
            boolean splitShipment =
                    rec.has("split_shipment") && rec.get("split_shipment").asBoolean();
            double distance =
                    rec.has("distance_miles") ? rec.get("distance_miles").asDouble() : 0.0;
            boolean hasFragile =
                    rec.has("has_fragile_high_value")
                            && rec.get("has_fragile_high_value").asBoolean();

            double savingsThreshold = logisticsConfig.getCostSavingsThreshold();
            double maxDistance = logisticsConfig.getLongHaulMiles();

            // Auto-approve: clear savings (costDelta <= -threshold), single warehouse, short haul,
            // not fragile
            if (costDelta <= -savingsThreshold
                    && !splitShipment
            ...
            // HITL triggers
            if (splitShipment) {
                return PolicyDecision.needsHuman("split_shipment_requires_ops")
                        .withAssigneeRole("routing");
            }
            if (distance >= maxDistance) {
                return PolicyDecision.needsHuman("long_haul_requires_review")
                        .withAssigneeRole("routing");
            }
            if (hasFragile) {
            ...
            if (costDelta > 0) {
                return PolicyDecision.needsHuman("cost_increase_requires_approval")
                        .withAssigneeRole("routing");
            }
            ...
            log.error(
                    "LogisticsPolicy: Failed to parse recommendation JSON for suggestion ID {}: {}",
                    s.getId(),
                    e.getMessage());
