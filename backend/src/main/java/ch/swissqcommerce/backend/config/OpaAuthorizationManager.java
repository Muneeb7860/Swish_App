...
/**
 * Dynamic policy enforcement manager. Queries a local Open Policy Agent (OPA) daemon for request
 * decisions with structured data. Falls back to static role-matching rules on connection failures.
 */
@Component
        ...
        if (opaEnabled) {
            try {
                ...
                ResponseEntity<Map> response =
                        restTemplate.postForEntity(opaUrl, payload, Map.class);
                if (response.getStatusCode() == HttpStatus.OK && response.getBody() != null) {
                    Map resultContainer = (Map) response.getBody().get("result");
                    if (resultContainer != null
                            && Boolean.TRUE.equals(resultContainer.get("allow"))) {
                        log.debug(
                                "OPA Authz [ALLOW]: user={}, uri={}, method={}",
                                username,
                                uri,
                                method);
                        return new AuthorizationDecision(true);
                ...
                log.warn(
                        "OPA Connection failed: {}. Falling back to Spring Security static rules.",
                        e.getMessage());
            }
        }
    ...
    private AuthorizationDecision evaluateFallbackRules(String uri, List<String> roles) {
        ...
        if (uri.startsWith("/api/orders")
            ...
            return new AuthorizationDecision(
                    roles.contains("ROLE_CUSTOMER") || roles.contains("ROLE_ADMIN"));
        }
