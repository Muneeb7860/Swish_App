...
public class SecurityConfig {
                        ...
                        auth ->
                                auth
                                        ...
                                        .requestMatchers(
                                                "/api/auth/**",
                                                "/api/v1/auth/**",
                                                "/api/v1/retailers/register", // FR-01: public
                                                // self-service
                                                // retailer signup
                                                "/api/v1/sensors/readings", // FR-03:
                                                // device-key-authenticated telemetry ingestion
                                                "/api/webhooks/**", // Phase 7: n8n webhook
                                                // callbacks
                                                // (HMAC-authenticated, not JWT)
                                                "/actuator/health",
                                                "/actuator/prometheus", // prometheus: scraped
                                                // without auth
                                                "/v3/api-docs/**",
                                                "/swagger-ui/**",
                                                "/swagger-ui.html",
                                                "/error" // Spring Boot error dispatch — must be
                                                // open for accessDeniedHandler to work
                                                )
                                        .permitAll()
                                        .requestMatchers("/api/**")
    ...
    public org.springframework.web.cors.CorsConfigurationSource corsConfigurationSource() {
        ...
        configuration.setAllowedOriginPatterns(
                java.util.Arrays.asList("http://localhost:[*]", "https://*.swissqcommerce.ch"));
