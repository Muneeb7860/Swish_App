 ...
 * governance items because the handlers lacked {@code @PreAuthorize}. Every handler
 * requires {@code ROLE_ADMIN}. The unified endpoints serve {@link HitlItem}s and
 * resolve by composite id via {@code resolveHitlItem}.
 *
 * <p>Full {@code @SpringBootTest} context so the app's real
 * {@code @EnableMethodSecurity} genuinely enforces the guard; filters are disabled
 * so we drive the role via a manually-set {@code SecurityContext}. The 403 on
 * denial and 409 on a double-resolve are produced by {@code GlobalExceptionHandler}.
 */
@SpringBootTest
...
public class HitlQueueControllerTest {
    ...
    void adminCanListPendingHitlItems() throws Exception {
        ...
        HitlItem pending = HitlItem.builder()
                .id("PA-1").source("B2B_PROCUREMENT").type("b2b_override")
                .status("PENDING").amount(new BigDecimal("6000.00"))
                .description("B2B restock override — wholesaler W1").build();
    ...
    void adminCanApprove() throws Exception {
        ...
        mockMvc.perform(post("/api/governance/hitl/PA-1/approve")
    ...
    void customerCannotApprove() throws Exception {
        ...
        verify(governanceUseCase, never()).resolveHitlItem(anyString(), anyBoolean(), anyString(), anyString());
    }
    ...
    void adminCanReject() throws Exception {
        ...
        mockMvc.perform(post("/api/governance/hitl/AQ-HITL-XYZ/reject")
                        .contentType(MediaType.APPLICATION_JSON)
        ...
        verify(governanceUseCase).resolveHitlItem("AQ-HITL-XYZ", false, "operator123", "Over budget");
    }
    ...
    void customerCannotReject() throws Exception {
        ...
        mockMvc.perform(post("/api/governance/hitl/PA-1/reject")
                        .contentType(MediaType.APPLICATION_JSON)
    ...
    void doubleApproveIsIdempotentConflict() throws Exception {
        ...
        doThrow(new TicketAlreadyResolvedException("Ticket is already APPROVED"))
                .when(governanceUseCase).resolveHitlItem("PA-1", true, "operator123", "High demand");
