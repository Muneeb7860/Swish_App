...
/**
 * Hardening regression tests for {@link CustomerController}'s object-level
 * authorization guard {@code assertOwnership(...)}.
 *
 * <p>Guards against the IDOR where a caller could place orders for, list the
 * orders of, refund the order of, or read the ledger of another customer by
 * passing a {@code customerId} (or operating on an order) they do not own. The
 * guard must reject any principal whose name != the target customerId unless it
 * carries ROLE_ADMIN; the JWT principal name == customerId by convention.
 *
 * <p>Covers the four guarded endpoints that previously had no controller-level
 * authz coverage. The fifth guarded endpoint, {@code purgeProfile}, is covered
 * separately in {@link CustomerControllerTest}. Each endpoint is exercised with
 * the owner / other-customer / admin matrix, and the side-effecting use-case
 * method is verified to never fire on an authz failure.
 *
 * <p>Filters are disabled so we exercise the controller's own ownership check in
 * isolation rather than the gateway/OPA URL-level policy. On the read endpoints
 * (which let the {@code AccessDeniedException} propagate rather than catching it
 * locally) the 403 is produced by {@code GlobalExceptionHandler}, which
 * {@code @WebMvcTest} loads as a {@code @RestControllerAdvice}.
 */
...
class CustomerControllerSecurityTest {
    ...
    void ownerCanPlaceOwnOrder() throws Exception {
        ...
        when(orderUseCase.checkout(eq(OWNER), anyList(), eq("Wallet"), any(BigDecimal.class), any(), any()))
    ...
    void ownerCanRefundOwnOrder() throws Exception {
        ...
        when(orderUseCase.requestRefund(eq(ORDER_ID), anyString(), any(BigDecimal.class), any(BigDecimal.class)))
                .thenReturn(Map.of("status", "refund_approved"));

        mockMvc.perform(post("/api/customer/orders/" + ORDER_ID + "/refund")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(REFUND_BODY))
                .andExpect(status().isOk());

        verify(orderUseCase, times(1))
                .requestRefund(eq(ORDER_ID), anyString(), any(BigDecimal.class), any(BigDecimal.class));
    }
