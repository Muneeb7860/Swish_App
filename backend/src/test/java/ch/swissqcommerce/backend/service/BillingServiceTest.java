...
class BillingServiceTest {
    ...
    private BillingAccount activeAccount(String id, BillingTier tier) {
        return BillingAccount.builder()
                .accountId(id).storeId("store-1").tier(tier).status("ACTIVE").build();
    }
    ...
    void subscribe_blankStore_rejected() {
        assertThrows(IllegalArgumentException.class, () -> service.subscribe("  ", BillingTier.BASIC));
        verify(port, never()).saveAccount(any());
    }

    @Test
    void generateInvoice_usesFlatTierAmount() {
        when(port.findAccountById("acc-1")).thenReturn(Optional.of(activeAccount("acc-1", BillingTier.PRO)));
        when(port.saveInvoice(any())).thenAnswer(i -> i.getArgument(0));

        Invoice inv = service.generateInvoice("acc-1", LocalDate.of(2026, 6, 1), LocalDate.of(2026, 6, 30));
    ...
    void generateInvoice_nonActiveAccount_rejected() {
        ...
        assertThrows(IllegalStateException.class,
                () -> service.generateInvoice("acc-2", LocalDate.of(2026, 6, 1), LocalDate.of(2026, 6, 30)));
        verify(port, never()).saveInvoice(any());
    }

    @Test
    void generateInvoice_invalidPeriod_rejected() {
        when(port.findAccountById("acc-3")).thenReturn(Optional.of(activeAccount("acc-3", BillingTier.BASIC)));

        assertThrows(IllegalArgumentException.class,
                () -> service.generateInvoice("acc-3", LocalDate.of(2026, 6, 30), LocalDate.of(2026, 6, 1)));
    }
    ...
    void generateInvoice_missingAccount_notFound() {
        ...
        assertThrows(java.util.NoSuchElementException.class,
                () -> service.generateInvoice("nope", LocalDate.of(2026, 6, 1), LocalDate.of(2026, 6, 30)));
    }

    @Test
    void markInvoicePaid_issuedBecomesPaid() {
        Invoice issued = Invoice.builder().invoiceId("inv-1").accountId("acc-1").status("ISSUED").build();
