...
public class AiOrchestrationServiceTest {
    ...
    @Mock
    private OpenAiChatModel cloudChatModel;
    ...
    public void testOrchestrateComplexTask_Failure() {
        when(cloudChatModel.stream("test prompt")).thenReturn(Flux.error(new RuntimeException("Cloud LLM offline")));
    ...
    public void testExecuteLocalTask_Failure() {
        when(localChatModel.stream("test prompt")).thenReturn(Flux.error(new RuntimeException("Local Ollama crashed")));
