...
public class FmcgCatalogServiceTest {
    ...
    public void testImportFmcgProducts_WithChaosParameters() {
        ...
        DynamicPricingAgent.PricingAnalysis pricingAnalysis =
                new DynamicPricingAgent.PricingAnalysis(
                        1.4, 5.0, 0.99, "Chaos price surge", 0.001, false);
        when(dynamicPricingAgent.recommendPricing(
                        eq(true), eq(0.4), anyDouble(), anyInt(), eq(0.25)))
                .thenReturn(pricingAnalysis);

        // Execute import with custom arguments
        List<FmcgImportResult> results = fmcgCatalogService.importFmcgProducts(true, 0.4, 0.25);

        assertNotNull(results);
        assertEquals(10, results.size());

        // Verify the recommendation was invoked with correct arguments
        verify(dynamicPricingAgent, times(10))
                .recommendPricing(eq(true), eq(0.4), anyDouble(), anyInt(), eq(0.25));
    }
}
