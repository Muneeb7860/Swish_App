...
public class OutcomeJobTest {
                ...
                status -> {
                    ...
                    AgentSuggestionEntity suggestion =
                            AgentSuggestionEntity.builder()
                                    ...
                                    .entityId("SKU-12345")
                                    .recommendation(
                                            "{\"action\":\"update_price\",\"old_value\":1250.00,\"new_value\":1392.50}")
                    ...
                    // 2. Create delivered order in the T+1 to T+7 window (T is now, so start =
                    // now+24h, end = now+7d)
                    // Seeding order 3 days in the future (OffsetDateTime.now().plusDays(3))
                    ch.swissqcommerce.backend.domain.transaction.adapter.out.persistence.OrderEntity
                            order =
    ...
    public void testOutcomeEvaluation_DegradedAlert() {
        ...
        // Run evaluation: all 3 should be evaluated as failed (actual revenue 0.0 < 1250.00
        // baseline)
        outcomeJob.runOutcomeEvaluation();
    ...
    public void testOutcomeEvaluation_PreCalculatedBaseline() {
                ...
                status -> {
                    ...
                    AgentSuggestionEntity suggestion =
                            AgentSuggestionEntity.builder()
                                    ...
                                    .recommendation(
                                            "{\"action\":\"update_price\",\"old_value\":10.00,\"new_value\":12.00}")
                    ...
                    ch.swissqcommerce.backend.domain.transaction.adapter.out.persistence
                            ...
                            orderItem =
                                    ch.swissqcommerce.backend.domain.transaction.adapter.out
                                            ...
                                            .price(
                                                    new BigDecimal(
                                                            "15.00")) // total 150.00 actual revenue
                                            .build();
                    entityManager.persist(orderItem);
        ...
        // Run evaluation: actual revenue is 150.00, baseline is 100.00 (from agent_baseline table),
        // delta = 50.00 > 0 (success)
        outcomeJob.runOutcomeEvaluation();
