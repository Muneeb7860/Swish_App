...
/**
 * Domain 8 Hardening — Integration tests verifying that all high-privilege operations write
 * immutable audit trails to the outbox and the security trust ledger, and that the compliance
 * report returns live data.
 */
@SpringBootTest
public class SecurityHardeningIntegrationTest {
    ...
    public void testHitlRejectionDeductsCustomerTrustAndWritesLedger() {
        ...
        Map<String, Object> result =
                adminService.resolveHitlTicket(ticketId, "reject", "Fraud confirmed");
        ...
        assertTrue(
                events.stream()
                        .anyMatch(
                                e ->
                                        "admin.hitl.resolve".equals(e.getEventType())
    ...
    public void testHitlApprovalWritesAuditOutboxEvent() {
        ...
        // Act
        Map<String, Object> result =
                adminService.resolveHitlTicket(ticketId, "approve", "Approved by ops");
    ...
    public void testGovernanceApproveOverrideWritesAuditOutbox() {
        ...
        B2BRestockOrderEntity restockEntity =
                transactionTemplate.execute(
                        status ->
                                restockOrderRepository.save(
                                        B2BRestockOrderEntity.builder()
                                                .wholesaler(
                                                        wholesalerRepository
                                                                .findById(WHOLESALER_ID)
                                                                .orElseThrow())
                                                .store(
                                                        darkStoreRepository
                                                                .findById("store-sec-1")
                                                                .orElseThrow())
        ...
        ProcurementApprovalEntity approval =
                transactionTemplate.execute(
                        status ->
                                approvalRepository.save(
                                        ProcurementApprovalEntity.builder()
        ...
        assertDoesNotThrow(
                () ->
                        governanceUseCase.approveOverride(
                                approval.getId(), "swissadmin", "Budget confirmed"));

        // Assert approval entity status updated
        ProcurementApprovalEntity updated =
                approvalRepository.findById(approval.getId()).orElseThrow();
        ...
        assertTrue(
                events.stream()
                        .anyMatch(
                                e ->
                                        "governance.override.approve".equals(e.getEventType())
                                                && "PENDING".equals(e.getStatus())),
    ...
    public void testGovernanceRejectOverrideWritesAuditOutbox() {
        ...
        assertDoesNotThrow(
                () ->
                        governanceUseCase.rejectOverride(
                                approval.getId(), "swissadmin", "Exceeds budget cap"));
        ...
        assertTrue(
                events.stream()
                        .anyMatch(
                                e ->
                                        "governance.override.reject".equals(e.getEventType())
                                                && "PENDING".equals(e.getStatus())),
    ...
    public void testComplianceReportReturnsLivePendingCounts() {
                ...
                status -> {
                    ...
                    outboxEventRepository.save(
                            OutboxEvent.builder()
                                    ...
                                    .payload(
                                            "{\"action\":\"security.anomaly\",\"operator\":\"swissadmin\",\"status\":\"FAILED\"}")
                                    .status("PENDING")
                                    .retryCount(0)
                                    .build());
                });
        ...
        assertEquals(
                1L,
                audit.get("pendingSecurityAnomalies"),
                "pendingSecurityAnomalies must reflect actual PENDING security.anomaly outbox"
                        + " events");
        assertEquals(
                0L,
                audit.get("analyzedSecurityAnomalies"),
    ...
    public void testHitlQueueControllerEndpointsEnforceAdminRole() {
        ...
        assertThrows(AccessDeniedException.class, () -> hitlQueueController.getPendingHitlItems());
        ...
        assertThrows(AccessDeniedException.class, () -> hitlQueueController.approve("1", req));

        // Accessing reject should throw AccessDeniedException
        assertThrows(AccessDeniedException.class, () -> hitlQueueController.reject("1", req));
        ...
        assertDoesNotThrow(() -> hitlQueueController.getPendingHitlItems());
    }
    ...
    public void testAdminControllerEndpointsEnforceAdminRole() {
        ...
        assertThrows(AccessDeniedException.class, () -> adminController.getActiveFaults());

        // Accessing AdminController.getOnboardingQueue should throw AccessDeniedException
        assertThrows(AccessDeniedException.class, () -> adminController.getOnboardingQueue());
        ...
        assertDoesNotThrow(() -> adminController.getActiveFaults());
        assertDoesNotThrow(() -> adminController.getOnboardingQueue());
    }
}
