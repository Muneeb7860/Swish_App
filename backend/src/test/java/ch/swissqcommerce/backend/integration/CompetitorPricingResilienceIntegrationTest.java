...
@SpringBootTest
public class CompetitorPricingResilienceIntegrationTest {

    @Autowired private CircuitBreakerRegistry circuitBreakerRegistry;

    @Autowired private CompetitorPricingClient competitorPricingClient;

    @Test
    public void testCompetitorApiCircuitBreaker_TripsOnFailure() throws Exception {
        CircuitBreaker circuitBreaker = circuitBreakerRegistry.circuitBreaker("competitor-api");
        assertNotNull(circuitBreaker, "competitor-api circuit breaker must be configured");
        ...
        Object originalHttpClient =
                ReflectionTestUtils.getField(competitorPricingClient, "httpClient");
        ReflectionTestUtils.setField(competitorPricingClient, "httpClient", mockHttpClient);
        ...
        try {
            ...
            for (int i = 0; i < 5; i++) {
                try {
                    competitorPricingClient.fetchCompetitorPrice("7613035449626");
            ...
            ReflectionTestUtils.setField(competitorPricingClient, "httpClient", originalHttpClient);
            circuitBreaker.reset();
        }
    }
}
