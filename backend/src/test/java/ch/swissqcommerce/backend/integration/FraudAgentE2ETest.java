...
public class FraudAgentE2ETest {
    ...
    void setUp() {
        ...
        transactionTemplate.executeWithoutResult(
                status -> {
                    // Native deletes first for FK-child tables (chargebacks -> order_items ->
                    // orders ->
                    // inventory) with no JPA repo cleared here; otherwise the dark_stores deleteAll
                    // below
                    // fails on a leftover inventory FK from a prior test (order-dependent pollution
                    // in the
                    ...
                    AgentRegistry riskAgentRegistry =
                            AgentRegistry.builder()
                                    .name("RiskAgent")
                                    .domain("risk")
                                    .version("1.0.0")
    ...
    public void testFullFraudLoop_WithAndWithoutChargeback() {
        ...
        when(riskAgent.analyze())
                .thenReturn(
                        AgentSuggestion.of(
                                "risk",
                                "hold_order order_id=" + order1.getOrderId() + " version=0",
                                0.95,
                                "suspicious pattern",
                                "high"));

        List<AgentSuggestionEntity> suggestions =
                agentOrchestrator.runOrchestrationSync("Fraud detection run 1");
        ...
        List<PolicyDecision> decisions1 =
                policyDecisionRepository.findBySuggestionIdOrderByIdDesc(suggestion1.getId());
        ...
        governanceService.resolveHitlItem(
                "AQ-" + ticket1.getTicketId(), true, "risk_analyst", "Approved fraud hold");
        ...
        when(riskAgent.analyze())
                .thenReturn(
                        AgentSuggestion.of(
                                "risk",
                                "hold_order order_id=" + order2.getOrderId() + " version=0",
                                0.95,
                                "suspicious pattern 2",
                                "high"));

        List<AgentSuggestionEntity> suggestions2 =
                agentOrchestrator.runOrchestrationSync("Fraud detection run 2");
        assertEquals(1, suggestions2.size());
        AgentSuggestionEntity suggestion2 =
                suggestions2.stream()
        ...
        HitlQueue ticket2 =
                hitlQueueRepository.findAll().stream()
        ...
        governanceService.resolveHitlItem(
                "AQ-" + ticket2.getTicketId(), true, "risk_analyst", "Approved second fraud hold");
