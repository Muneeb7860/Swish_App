...
public class SecurityAnomalyIntegrationTest {
    ...
    @Autowired private SecurityAnomalyAnalyzer securityAnomalyAnalyzer;
    ...
    public void testSuccessfulActionGeneratesAuditTrail() {
        ...
        assertTrue(
                events.stream()
                        .anyMatch(
                                e ->
                                        "admin.chaos.inject".equals(e.getEventType())
    ...
    public void testExceptionGeneratesSecurityAnomalyAndRunsAiAnalysis() {
        ...
        assertThrows(
                ...
                () -> {
                    adminService.injectFault(
                            "INVALID_FAULT_LOG", "Should fail and trigger anomaly logging");
                });

        // Verify outbox contains anomaly event
        List<OutboxEvent> eventsAfterAnomaly = outboxEventRepository.findAll();
        assertTrue(
                eventsAfterAnomaly.stream()
                        .anyMatch(
                                e ->
                                        "security.anomaly".equals(e.getEventType())
                                                && "PENDING".equals(e.getStatus())));
        ...
        assertTrue(
                eventsAfterAnalysis.stream()
                        .anyMatch(
                                e ->
                                        "security.anomaly".equals(e.getEventType())
                                                && "ANALYZED".equals(e.getStatus())));
    }
    ...
    public void testSchemaValidationFailureDLQ() {
        // Insert a malformed outbox event that fails schema requirements (missing required field:
        // 'operator')
        transactionTemplate.executeWithoutResult(
                status -> {
                    OutboxEvent malformedEvent =
                            OutboxEvent.builder()
                                    .aggregateType("SECURITY")
                                    .aggregateId("swissadmin")
                                    .eventType("security.anomaly")
                                    .payload(
                                            "{\"action\":\"security.anomaly\",\"method\":\"injectFault\",\"timestamp\":\"2026-06-04T12:00:00Z\",\"status\":\"FAILED\",\"error\":\"Anomaly"
