 ...
 *
 * Addresses the O'Reilly "AI Agents Stack 2026" eval gap:
 * validates routing accuracy, HITL calibration, budget guardrails, RFQ auction
 * winner selection, and Prometheus observability — all without a live LLM.
 *
 * MasterOrchestratorService is the real Spring bean; only the LLM adapters
 * (CustomerSupportAgent, B2BProcurementAgent, ProcurementGuardrailsEngine)
 * are mocked to keep tests hermetic.
 *
 * IMPORTANT: evalBudgetGuardrail runs last (@Order 99) because it leaves
 * MasterOrchestratorService.dailyCost = 6.0 on the singleton, which would
 * poison earlier tests if they ran in a different order.
 */
@SpringBootTest
...
public class AgentEvalIntegrationTest {
    ...
    @BeforeEach
    void resetMocks() {
        reset(customerSupportAgent, b2BProcurementAgent, procurementGuardrailsEngine,
              agentOutPort, eventUseCase, wholesalerPort, restockOrderPort, governanceUseCase);
        ...
        // publishEvent returns DomainEventEntity — mock returns null; result is never used by caller
        when(eventUseCase.publishEvent(anyString(), anyString())).thenReturn(null);
        // findCustomerById returns Optional.empty by default (Mockito default); override in tests that need a customer
        ...
        when(procurementGuardrailsEngine.validate(anyDouble(), anyDouble(), anyInt()))
                .thenReturn(new ProcurementGuardrailsEngine.GuardrailResult(true, "Auto-approved in test"));
    }
    ...
    public void evalToolSelection_OrderQuery_RoutesToOrderStatusTool() {
        ...
        CustomerSupportAgent.AgentAnalysis firstPass = new CustomerSupportAgent.AgentAnalysis(
                "Let me check that for you.", 0.80, "ORDER_STATUS", "42", 0.01
        );
        CustomerSupportAgent.AgentAnalysis finalPass = new CustomerSupportAgent.AgentAnalysis(
        ...
        AgentRequest req = new AgentRequest("What is the status of my order 42?", "conv-1", "cust-1");
        AgentResponse response = agentUseCase.processMessage(req);

        // Verify tool execution branch was taken
        verify(customerSupportAgent, times(1)).generateFinalResponse(any(), anyString(), anyDouble());
    ...
    public void evalHitlCalibration_ConfidenceBelow70_EscalatesHitl() {
        ...
        Customer customer = Customer.builder()
                .customerId("cust-3").fullName("Eva Test").hashedEmail("hash3").build();
        ...
        assertTrue(cap.getValue().getDescription().contains("Low confidence score: 0.65"),
                "Ticket description must mention confidence score. Actual: " + cap.getValue().getDescription());
    }
    ...
    public void evalNegotiation_LowestBidWins_RfqAuction() {
        Wholesaler cheapWholesaler = Wholesaler.builder()
                .wholesalerId("w-cheap").name("BestPrice Supplier")
                .isActive(true).trustScore(75).build();
        Wholesaler expensiveWholesaler = Wholesaler.builder()
                .wholesalerId("w-exp").name("PremiumSupply AG")
        ...
        assertNotNull(resp);
        assertTrue(resp.getMessage().contains("BestPrice Supplier"),
    ...
    public void evalNegotiation_TrustScoreTiebreaker_HigherTrustWins() {
        Wholesaler lowTrust  = Wholesaler.builder().wholesalerId("w-lt").name("LowTrust Corp")
                .isActive(true).trustScore(50).build();
        Wholesaler highTrust = Wholesaler.builder().wholesalerId("w-ht").name("HighTrust AG")
                .isActive(true).trustScore(90).build();
        ...
        assertNotNull(resp);
        assertTrue(resp.getMessage().contains("HighTrust AG"),
                "When prices are equal, higher trust score must win. Message: " + resp.getMessage());
    }
    ...
    public void evalBudgetGuardrail_DailyBudgetExceeded_BlocksSubsequentRequest() {
        ...
        assertEquals("System limit reached. Your request is routed to a customer support agent.",
                blocked.getReply(), "Budget guardrail must block requests after $5 daily limit");
