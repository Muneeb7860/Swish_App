...
public class RoutingAgentIntegrationTest {
    ...
    void setUp() {
        ...
        transactionTemplate.executeWithoutResult(
                status -> {
                    ...
                    WarehouseBaseline wb =
                            WarehouseBaseline.builder()
                                    .zipPrefix("800")
    ...
    public void testFullRoutingLoop_WithSplitShipment() {
        ...
        when(routingAgent.analyze())
                .thenReturn(
                        AgentSuggestion.of(
                                "routing",
                                suggestionJson,
                                0.90,
                                "optimize split-shipment route",
                                "high"));

        // Run orchestration
        List<AgentSuggestionEntity> suggestions =
                agentOrchestrator.runOrchestrationSync("Logistics routing run");
        ...
        List<PolicyDecision> decisions =
                policyDecisionRepository.findBySuggestionIdOrderByIdDesc(suggestionEntity.getId());
        ...
        governanceService.resolveHitlItem(
                "AQ-" + ticket.getTicketId(), true, "routing", "Approved logistics route");

        // Verify execution results: order is updated with primary warehouse and shipments are
        // logged
        entityManager.flush();
        entityManager.clear();
        ...
        transactionTemplate.executeWithoutResult(
                status -> {
                    entityManager
                            .createNativeQuery(
                                    "UPDATE oltp.shipments SET actual_shipping_cost = 3.00 WHERE"
                                            + " warehouse_id = 'WH-NY-01'")
                            .executeUpdate();
                    entityManager
                            .createNativeQuery(
                                    "UPDATE oltp.shipments SET actual_shipping_cost = 4.00 WHERE"
                                            + " warehouse_id = 'WH-CA-02'")
                            .executeUpdate();
                });
        ...
        // actual sum = 9.00
        // wait! Since baseline was ~8.17, actual is 9.00, wait, savings = 8.17 - 9.00 = -0.83
        // (negative savings).
