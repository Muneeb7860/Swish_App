...
public class PaymentIntegrationTest {
    ...
    public void testAuthorizeAndCapturePaymentFlow() {
        List<OrderUseCase.CartItem> items = List.of(
                new OrderUseCase.CartItem("BREAD-001", 2)
        );
        ...
        Payment authorization = paymentUseCase.authorizePayment(
                order.getOrderId(),
                customer.getCustomerId(),
    ...
    public void testAuthorizePaymentIsIdempotent() {
        List<OrderUseCase.CartItem> items = List.of(
                new OrderUseCase.CartItem("BREAD-001", 1)
        );
        String orderIdempotencyKey = UUID.randomUUID().toString();
        ...
        Payment firstPayment = paymentUseCase.authorizePayment(
                order.getOrderId(),
                customer.getCustomerId(),
                order.getTotalAmount(),
                "Wallet",
        ...
        Payment secondPayment = paymentUseCase.authorizePayment(
                order.getOrderId(),
                customer.getCustomerId(),
                order.getTotalAmount(),
                "Wallet",
