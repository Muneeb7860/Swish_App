...
public class DispatchIntegrationTest {
    ...
    @Autowired private GearScanRepository gearScanRepository;

    @Autowired private ActiveShipmentRepository activeShipmentRepository;

    @Autowired private VehicleConfigRepository vehicleConfigRepository;
    ...
    void setUp() {
        ...
        vehicleConfigRepository.save(
                ch.swissqcommerce.backend.domain.dispatch.adapter.out.persistence
                        .VehicleConfigEntity.builder()
                        .vehicleType("E-Bike")
        ...
        riderRepository.save(
                ch.swissqcommerce.backend.domain.enrollment.adapter.out.persistence.RiderEntity
                        .builder()
                        ...
                        .build());
        normalRider =
                ch.swissqcommerce.backend.domain.enrollment.core.model.Rider.builder()
                        .riderId("RIDER-EBike-1")
                        .build();
    ...
    public void testAssignOrder_FailsWhenWeightExceedsCapacity() {
        ...
        dispatchUseCase.submitGearScan(
                normalRider.getRiderId(), "THERMAL_BAG", "PASSED", "http://image.url/bag.jpg");
        ...
        assertThrows(
                ...
                () -> {
                    dispatchUseCase.assignOrder(
                            order.getOrderId(), normalRider.getRiderId(), new BigDecimal("15.00"));
                });
    }
    ...
    public void testAssignOrder_FailsOnSelfMatching() {
        ...
        dispatchUseCase.submitGearScan(
                selfMatchingRider.getRiderId(),
                "THERMAL_BAG",
                "PASSED",
                "http://image.url/bag.jpg");
    ...
    public void testAssignOrder_SucceedsWithWeightTolerance() {
        ...
        // Act & Assert (Weight of 10.50kg exceeds E-Bike limit of 10.00kg, but is within 10%
        // tolerance (11.00kg))
        ActiveShipment shipment =
                dispatchUseCase.assignOrder(
                        order.getOrderId(), normalRider.getRiderId(), new BigDecimal("10.50"));
        assertNotNull(shipment);
        assertEquals("ASSIGNED", shipment.getStatus().name());
    }
    ...
    public void testStationaryGPSReallocation() {
        ...
        dispatchUseCase.updateRiderGps(
                normalRider.getRiderId(), new BigDecimal("47.3769"), new BigDecimal("8.5417"));

        // Send second GPS update to the exact same position (flags as stationary)
        shipment =
                dispatchUseCase.updateRiderGps(
                        normalRider.getRiderId(),
                        new BigDecimal("47.3769"),
                        new BigDecimal("8.5417"));
        assertNotNull(shipment.getStationarySince());

        // Manually adjust the stationarySince timestamp back in database by 11 minutes (simulating
        // time elapsed)
        ch.swissqcommerce.backend.domain.dispatch.adapter.out.persistence.ActiveShipmentEntity
                shipmentEntity =
                        activeShipmentRepository.findById(shipment.getShipmentId()).orElseThrow();
