...
public class RoutingCapacityIntegrationTest {
    ...
    public void testOptimalWarehouseDisqualifiesExceededCapacity() {
        ...
        RoutingOrderData orderData = new RoutingOrderData(
                ...
                addr,
                null,
                List.of(new RoutingOrderData.OrderItem("item-1-NY", 1))
        );

        // 1. Initially, WH-NY-01 has daily capacity limit 1 and 0 orders today.
        // Fulfills the order successfully.
        Optional<WarehouseSelectionService.RoutingResult> resultOpt1 = selectionService.findOptimalWarehouse(orderData);
        ...
        Optional<WarehouseSelectionService.RoutingResult> resultOpt2 = selectionService.findOptimalWarehouse(orderData);
        assertFalse(resultOpt2.isPresent()); // Returns empty since the only stocked warehouse NY is over capacity
    }
}
