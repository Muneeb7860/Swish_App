...
/**
 * Hardening regression tests for the payment controller's object-level authorization guard (domain
 * 9, E-Commerce Core).
 *
 * <p>Guards against the IDOR where a caller could authorize, list, or read payments belonging to
 * another customer by passing an arbitrary {@code customerId} (or payment id) that they do not own.
 * The controller's {@code assertPaymentOwnership(...)} check must reject any principal whose name
 * != the payment's customerId unless it carries ROLE_ADMIN.
 *
...
class PaymentControllerSecurityTest {
    ...
    @MockBean private PaymentUseCase paymentUseCase;

    @MockBean private PaymentProcessingUseCase paymentProcessingUseCase;
    ...
    void ownerCanAuthorizeOwnPayment() throws Exception {
        ...
        when(paymentUseCase.authorizePayment(
                        anyInt(), eq(OWNER), any(BigDecimal.class), anyString(), any()))
    ...
    void ownerCanListOwnPayments() throws Exception {
        ...
        mockMvc.perform(get("/api/payments").param("customerId", OWNER)).andExpect(status().isOk());
    ...
    void cannotGetAnotherCustomersPayment() throws Exception {
        ...
        mockMvc.perform(get("/api/payments/1")).andExpect(status().isForbidden());
    }
