...
| **R5** ✅ | Distributed hybrid agentic | **Bridge built**: `GovernedAgentPort` + `GovernanceBridgeAdapter` → Python `homelab-ai-governance` `POST /api/v1/govern` (config `swish.governance.api.url`), fail-safe-local fallback; `GovernedAgentController` exposes `/api/v1/agent/governance/route`. Python `server.py` endpoint already present. Live e2e (Java→Python→Ollama) gated on both services running. |
