...
| FR-03 | Telemetry ingestion (MQTT/HTTPS → **TimescaleDB**) | order cold-chain CQRS on PostgreSQL + **`sensor_readings` TimescaleDB hypertable** (V25) with device-key HTTP ingestion (`sensor` context). **Architect's Decision:** HTTPS standardized as final pilot protocol; MQTT deferred indefinitely to avoid infrastructure bloat. Cryptographic telemetry hash-chain chaining (SHA-256) at ingestion + continuous sensor calibration tracking with dynamic B2B restock order rerouting. | ✅ | R3 (HTTPS standard) |
...
| **R5** ✅ | Distributed hybrid agentic | **Canonical impl (parallel agent):** `PythonGovernanceAdapter` (implements `LlmGatewayPort`) routes the customer-support, B2B-procurement, and dynamic-pricing agents through the Python `homelab-ai-governance` REST service (`POST /api/v1/govern`, config `swish.governance.api.url`), with `GeminiFreeAdapter`/`MockLlmAdapter` offline fallback. Python `server.py` + `cli.py` expose the service. |
| **R6** ✅ | Memory, Mesh & Security | **Stateful Agent memory & collaboration:** In-memory Agent Mesh tool routing (with token cost limits) + Letta (MemGPT) agent memory client + pgvector RAG grounding on Python governance side. Method security hardened via `@PreAuthorize("hasRole('ADMIN')")` on all HITL/Admin endpoints. One-click UAT demo orchestration script `run_demo.sh` deployed in workspace root. |
