            ...
            {!activeOrder ? (
              ...
              <div className="glass-card" style={{ padding: '1.25rem' }}>
                <h4 style={{ fontWeight: 700, color: 'var(--color-rider)', marginBottom: '0.75rem' }}>
                  LIVE TRANSIT DISPATCH: Order #{activeOrder.id} ({activeOrder.status ? activeOrder.status.toUpperCase() : 'TRANSIT'})
                </h4>
                
                <div style={{ display: 'flex', gap: '1.25rem', marginTop: '1rem', flexDirection: 'column' }}>
                  
                  {activeOrder.status === 'arrived' ? (
                    <div className="glass-card" style={{ padding: '1rem', background: 'rgba(16, 185, 129, 0.05)', border: '1px dashed var(--color-rider)', borderRadius: '8px' }}>
                      <h5 style={{ fontWeight: 800, color: 'var(--color-rider)', display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.5rem' }}>
                        <Lucide.CheckCircle size={18} /> 📍 Arrived at Destination Doorstep!
                      </h5>
                      <p style={{ fontSize: '0.7rem', color: 'var(--text-secondary)', marginBottom: '1rem' }}>
                        Swiss regulatory compliance requires a cryptographic Proof-of-Delivery (PoD) final handshake. Scan the customer's QR code or upload a doorstep photo.
                      </p>
                      
                      <div style={{ display: 'flex', gap: '0.75rem', marginBottom: '1rem' }}>
                        <button 
                          className="btn-secondary-glow" 
                          style={{ flex: 1, fontSize: '0.75rem', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.25rem', cursor: 'pointer', padding: '0.5rem' }}
                          onClick={() => {
                            setProofType('photo');
                      ...
                      {podHash && (
                        <div style={{ background: '#020408', padding: '0.5rem', borderRadius: '4px', marginBottom: '1rem', border: '1px solid rgba(255,255,255,0.05)' }}>
                          ...
                          <div style={{ fontFamily: 'var(--font-mono)', fontSize: '0.65rem', wordBreak: 'break-all', marginTop: '0.25rem', color: 'var(--color-rider)' }}>
                            {podHash}
                          </div>
                        </div>
                      )}
                      ...
                      <button 
                        className="btn-primary-glow" 
                        disabled={!podHash}
                        style={{ width: '100%', background: podHash ? 'var(--color-rider)' : '#27293d', color: podHash ? '#070a13' : 'var(--text-muted)', border: 'none', padding: '0.6rem', cursor: podHash ? 'pointer' : 'not-allowed', fontWeight: 800, fontSize: '0.75rem' }}
                        onClick={() => {
