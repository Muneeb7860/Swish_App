 ...
 *   3. GET  /api/orders — verify order appears in order history
 *   4. POST /api/orders (same idempotency key) — idempotency guard returns same order
 *   5. POST /api/orders/{id}/refund — request a refund
 *
...
describe("Order placement — API", () => {
  ...
  it("POST /api/orders — places a checkout order and returns 200 with orderId", () => {
  ...
  it("GET /api/orders — authenticated customer can retrieve their order history", () => {
    cy.request({
      method: "GET",
      url: `${API()}/api/orders`,
      headers: { Authorization: `Bearer ${token}` },
  ...
  it("POST /api/orders — same idempotency key returns 200 without duplicating the order", () => {
    // Skip if the first checkout failed (item not seeded)
    if (!createdOrderId) {
      cy.log("Skipping idempotency check — initial checkout did not produce an order");
      return;
    }
    ...
    cy.request<OrderResponse>({
      ...
      expect(res.body.orderId).to.eq(createdOrderId,
  ...
  it("GET /api/orders — returns 401 without a bearer token", () => {
    cy.request({
      method: "GET",
      url: `${API()}/api/orders`,
      failOnStatusCode: false,
  ...
  it("POST /api/orders — returns 401 without a bearer token", () => {
    cy.request({
      method: "POST",
      url: `${API()}/api/orders`,
      body: { items: [], paymentMethod: "wallet", idempotencyKey: "unauthenticated" },
      failOnStatusCode: false,
    }).then((res) => {
      expect(res.status).to.eq(401);
    });
