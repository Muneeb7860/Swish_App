...
describe("Rider delivery — API", () => {
  ...
  it("POST /api/rider/onboard — submits onboarding application", () => {
    cy.request({
      ...
      cy.task("log", `Rider onboard status: ${res.status} — ${JSON.stringify(res.body).slice(0, 120)}`);
    });
  });
  ...
  it("POST /api/rider/orders/99999/deliver — non-existent order returns 404 or 403", () => {
    ...
    cy.request({
      ...
      expect(res.status).to.be.oneOf([400, 403, 404, 422],
  ...
  it("full flow: onboard → approve → assign order → deliver (requires CYPRESS_ADMIN_TOKEN)", () => {
    ...
    if (!adminToken) {
      cy.log("⚠ CYPRESS_ADMIN_TOKEN not set — skipping full delivery flow test");
      return;
    }
    ...
    cy.request({
      ...
      const riderApp = queue.find((a: any) =>
        a.email === riderEmail || a.fullName === "E2E TestRider"
      );

      if (!riderApp) {
        cy.log("Rider onboarding application not found in admin queue — skipping");
        return;
      }
