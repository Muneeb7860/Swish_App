  ...
  useEffect(() => {
    ...
    if (isViteEnv && import.meta.env.VITE_APP_ENV) {
      ...
      setEnvProfiles(prev => {
        ...
        target.logLevel = (import.meta.env.VITE_LOG_LEVEL || target.logLevel).toLowerCase();
        target.mfaRequired = import.meta.env.VITE_MFA_REQUIRED === 'true' || import.meta.env.VITE_MFA_REQUIRED === true;
        target.rateLimit = parseInt(import.meta.env.VITE_RATE_LIMIT, 10) || target.rateLimit;
        target.dbLatencyDefault = parseInt(import.meta.env.VITE_DB_LATENCY_DEFAULT, 10) || target.dbLatencyDefault;
        target.restockVelocityDefault = parseInt(import.meta.env.VITE_RESTOCK_VELOCITY_DEFAULT, 10) || target.restockVelocityDefault;
        target.circuitBreakerLimit = parseInt(import.meta.env.VITE_CIRCUIT_BREAKER_LIMIT, 10) || target.circuitBreakerLimit;
    ...
    async function loadEnvFiles() {
      try {
        ...
        setEnvProfiles(prev => {
          const updated = { ...prev };
          const target = updated[profileKey] || (updated[profileKey] = { envName: profileKey });
