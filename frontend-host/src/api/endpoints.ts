...
export const login = (data: LoginRequest): Promise<LoginResponse> => {
	return ApiClient.post<LoginResponse>(
		...
		{ bypassMock: false },
		() => {
			const loginName = data.username || (data.email ? data.email.split("@")[0] : "");
			if (loginName === "admin") {
				return { mfaRequired: true, mfa_required: true, sessionToken: "mock-sess-admin", session_token: "mock-sess-admin" };
			} else if (loginName === "rider") {
				return { mfaRequired: true, mfa_required: true, sessionToken: "mock-sess-rider", session_token: "mock-sess-rider" };
			} else if (loginName === "inventory") {
				return { mfaRequired: true, mfa_required: true, sessionToken: "mock-sess-inventory", session_token: "mock-sess-inventory" };
			} else if (loginName === "wholesaler") {
				return { mfaRequired: true, mfa_required: true, sessionToken: "mock-sess-wholesaler", session_token: "mock-sess-wholesaler" };
			}
...
export const verifyMfa = (data: MfaVerifyRequest): Promise<MfaVerifyResponse> => {
	...
	return ApiClient.post<Order>(
		...
		() => {
			const total = order.items.reduce((sum, item) => {
				const prod = mocks.MOCK_PRODUCTS.find((p) => p.item_id === item.item_id);
...
export const requestRefund = (id: number, request: RefundRequest): Promise<RefundResponse> => {
...
export const gdprPurge = (): Promise<{ status: string; probationary_trust_score: number }> => {
...
export const submitOnboard = (onboard: RiderOnboardRequest): Promise<RiderOnboardResponse> => {
...
export const injectCoolant = (id: number): Promise<{ message: string; new_temperature: number }> => {
...
export const confirmDelivery = (id: number, request: { pin?: string; photoUrl?: string }): Promise<any> => {
	return ApiClient.post<any>(
...
export const rejectDelivery = (id: number, request: { reason: string; photoUrl: string }): Promise<any> => {
	return ApiClient.post<any>(
...
export const getPickerQueue = (): Promise<Order[]> => {
	return ApiClient.get<Order[]>(
		"/api/inventory/picker/queue",
		{},
		() => mocks.MOCK_ORDERS.filter((o) => o.status === "picking" || o.status === "pending")
	);
};

export const handoverItem = (data: PickerHandoverRequest): Promise<PickerHandoverResponse> => {
...
export const rebalanceInventory = (data: RebalanceRequest): Promise<RebalanceResponse> => {
...
export const fulfillRestock = (id: number): Promise<{ status: string; invoice_amount: number }> => {
...
export const injectChaosFault = (data: ChaosFaultRequest): Promise<ChaosFaultResponse> => {
...
export const approveOnboard = (id: string, data: OnboardingApproveRequest): Promise<OnboardingApplication> => {
	const body = {
	...
	return ApiClient.post<OnboardingApplication>(
		...
		() => ({
			...
			approval_compliance: data.validator_role === "Compliance" ? data.approve : false,
...
export const resolveHitl = (id: string, data: HitlResolveRequest): Promise<HitlTicket> => {
	const body = {
...
export const approveB2bOverride = (id: number, request: { operator: string; reason: string }): Promise<any> => {
	return ApiClient.post<any>(
...
export const rejectB2bOverride = (id: number, request: { operator: string; reason: string }): Promise<any> => {
	return ApiClient.post<any>(
