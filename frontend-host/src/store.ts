...
export const useStore = create<State>((set) => ({
  ...
  customerPoints: 45,
  setCustomerPoints: (val) => set((state: any) => ({ customerPoints: typeof val === 'function' ? val(state.customerPoints) : val } as any)),
  riderWallet: 15.00,
  setRiderWallet: (val) => set((state: any) => ({ riderWallet: typeof val === 'function' ? val(state.riderWallet) : val } as any)),
  merchantWallet: 1542.80,
  setMerchantWallet: (val) => set((state: any) => ({ merchantWallet: typeof val === 'function' ? val(state.merchantWallet) : val } as any)),
  customerOrderCount: 2,
  setCustomerOrderCount: (val) => set((state: any) => ({ customerOrderCount: typeof val === 'function' ? val(state.customerOrderCount) : val } as any)),
  customerRefundCount: 0,
  setCustomerRefundCount: (val) => set((state: any) => ({ customerRefundCount: typeof val === 'function' ? val(state.customerRefundCount) : val } as any)),
  customerTab: 'catalog',
  setCustomerTab: (val) => set((state: any) => ({ customerTab: typeof val === 'function' ? val(state.customerTab) : val } as any)),
  profileSubTab: 'vip',
  setProfileSubTab: (val) => set((state: any) => ({ profileSubTab: typeof val === 'function' ? val(state.profileSubTab) : val } as any)),
  savedAddresses: [
  ...
  setSavedAddresses: (val) => set((state: any) => ({ savedAddresses: typeof val === 'function' ? val(state.savedAddresses) : val } as any)),
  savedCards: [
    { id: 'c1', bank: 'Visa Premium Credit Card', number: '•••• •••• •••• 9823', expiry: '12/28' },
    { id: 'c2', bank: 'Mastercard Gold Debit', number: '•••• •••• •••• 4120', expiry: '05/29' }
  ],
  setSavedCards: (val) => set((state: any) => ({ savedCards: typeof val === 'function' ? val(state.savedCards) : val } as any)),
  favorites: ['Organic Fresh Milk', 'Chiquita Bananas (1kg)', 'Fresh Hass Avocado (Pair)'],
  setFavorites: (val) => set((state: any) => ({ favorites: typeof val === 'function' ? val(state.favorites) : val } as any)),
  vipMember: true,
  setVipMember: (val) => set((state: any) => ({ vipMember: typeof val === 'function' ? val(state.vipMember) : val } as any)),
  vouchers: [
  ...
  setCurrentUserSession: (val) => set((state: any) => ({ currentUserSession: typeof val === 'function' ? val(state.currentUserSession) : val } as any)),
  mfaStep: 'credentials',
  setMfaStep: (val) => set((state: any) => ({ mfaStep: typeof val === 'function' ? val(state.mfaStep) : val } as any)),
  mfaRole: 'customer',
  setMfaRole: (val) => set((state: any) => ({ mfaRole: typeof val === 'function' ? val(state.mfaRole) : val } as any)),
  mfaPassword: '',
  setMfaPassword: (val) => set((state: any) => ({ mfaPassword: typeof val === 'function' ? val(state.mfaPassword) : val } as any)),
  mfaOtpSentCode: null,
  setMfaOtpSentCode: (val) => set((state: any) => ({ mfaOtpSentCode: typeof val === 'function' ? val(state.mfaOtpSentCode) : val } as any)),
  mfaOtpInput: '',
  setMfaOtpInput: (val) => set((state: any) => ({ mfaOtpInput: typeof val === 'function' ? val(state.mfaOtpInput) : val } as any)),
  mfaMethod: 'sms',
  setMfaMethod: (val) => set((state: any) => ({ mfaMethod: typeof val === 'function' ? val(state.mfaMethod) : val } as any)),
  totpSecretCode: '',
  setTotpSecretCode: (val) => set((state: any) => ({ totpSecretCode: typeof val === 'function' ? val(state.totpSecretCode) : val } as any)),
  totpTimer: 30,
  setTotpTimer: (val) => set((state: any) => ({ totpTimer: typeof val === 'function' ? val(state.totpTimer) : val } as any)),
  sessionToken: '',
  ...
  setActiveTrainingRole: (val) => set((state: any) => ({ activeTrainingRole: typeof val === 'function' ? val(state.activeTrainingRole) : val } as any)),
  trainingProgress: 0,
  ...
  setWholesalerTrustScore: (val) => set((state: any) => ({ wholesalerTrustScore: typeof val === 'function' ? val(state.wholesalerTrustScore) : val } as any)),
  trustLogs: [
    { id: 'T0', time: new Date().toLocaleTimeString(), actor: 'system', event: 'Security trust systems initialized', delta: 0, current: 100 }
  ],
  setTrustLogs: (val) => set((state: any) => ({ trustLogs: typeof val === 'function' ? val(state.trustLogs) : val } as any)),
  coldChainBreakdownActive: false,
  setColdChainBreakdownActive: (val) => set((state: any) => ({ coldChainBreakdownActive: typeof val === 'function' ? val(state.coldChainBreakdownActive) : val } as any)),
  wholesalerOutageActive: false,
  setWholesalerOutageActive: (val) => set((state: any) => ({ wholesalerOutageActive: typeof val === 'function' ? val(state.wholesalerOutageActive) : val } as any)),
  paymentOutageActive: false,
  setPaymentOutageActive: (val) => set((state: any) => ({ paymentOutageActive: typeof val === 'function' ? val(state.paymentOutageActive) : val } as any)),
  ...
  eastCapacity: 120,
  setEastCapacity: (val) => set((state: any) => ({ eastCapacity: typeof val === 'function' ? val(state.eastCapacity) : val } as any)),
  ...
  setAgentMetrics: (val) => set((state: any) => ({ agentMetrics: typeof val === 'function' ? val(state.agentMetrics) : val } as any)),
  oltpWriteLatency: 4,
  ...
  setLatencyHistory: (val) => set((state: any) => ({ latencyHistory: typeof val === 'function' ? val(state.latencyHistory) : val } as any)),
  cacheHits: 14,
  setCacheHits: (val) => set((state: any) => ({ cacheHits: typeof val === 'function' ? val(state.cacheHits) : val } as any)),
  cacheMisses: 2,
  setCacheMisses: (val) => set((state: any) => ({ cacheMisses: typeof val === 'function' ? val(state.cacheMisses) : val } as any)),
  circuitBreakerTripped: false,
  setCircuitBreakerTripped: (val) => set((state: any) => ({ circuitBreakerTripped: typeof val === 'function' ? val(state.circuitBreakerTripped) : val } as any)),
  rateLimitActive: false,
  setRateLimitActive: (val) => set((state: any) => ({ rateLimitActive: typeof val === 'function' ? val(state.rateLimitActive) : val } as any)),
  kafkaLogs: [
  ...
  setKafkaLogs: (val) => set((state: any) => ({ kafkaLogs: typeof val === 'function' ? val(state.kafkaLogs) : val } as any)),
  ledger: [
