...
export default function RiderScreen() {
	...
	const [jwtToken, setJwtToken] = useState(
		"eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiJyaWRlcl9kYXZlIiwicm9sZXMiOiJSSURVUiIsImlhdCI6MTc4MDAwMDAwMH0.signature",
	);
		...
		if (deliveryState === "in_transit") {
			...
			tickTimerRef.current = setInterval(() => {
				...
				setTemperature((prevTemp) => {
					const tempIncrease = 0.1 + Math.random() * 0.15; // Random increase
					...
					if (isSpoiled) {
						addTelemetryLog(
							`[${timeStr}] 🚨 CRITICAL BREACH: Cargo temp hit ${newTemp}°C. Perishables ruined!`,
							"error",
						);
						Alert.alert(
							"Cold Chain Breach",
							`Cargo temperature reached ${newTemp}°C. Perishables are spoiled! Delivery failed.`,
						);
						setDeliveryState("incident_blocked");
						setRiderStats((prev) => ({
							...prev,
							trustScore: Math.max(50, prev.trustScore - 20),
						}));
					} else if (isBreach) {
						addTelemetryLog(
							`[${timeStr}] ⚠️ THERMAL ALERT: Cargo warming up at ${newTemp}°C!`,
							"warning",
						);
					} else {
						addTelemetryLog(
							`[${timeStr}] Telemetry updated. Temp: ${newTemp}°C | Humidity: ${humidity}%`,
							"normal",
						);
					}
			...
			routeTimerRef.current = setInterval(() => {
				setRouteProgress((prev) => {
					...
					if (next >= 100) {
						...
						addTelemetryLog(
							"Rider arrived at customer destination geofence.",
							"info",
						);
						return 100;
					}
					...
					setGpsCoords((prevGps) => {
						const dest = activeOrder
							? activeOrder.gpsDestination
							: MOCK_ORDER.gpsDestination;
						const start = activeOrder
							? activeOrder.gpsStart
							: MOCK_ORDER.gpsStart;
	...
	const testBffConnection = async () => {
		...
		try {
			...
			if (response.ok) {
				...
				Alert.alert(
					"Gateway Connected",
					"Connected to BFF Gateway. JWT authentication validated successfully.",
				);
	...
	const handleBffLogin = async () => {
		...
		try {
			...
			if (response.ok) {
				...
				if (data.mfaRequired || data.sessionToken) {
					...
					Alert.alert(
						...
						`Login successful! MFA PIN code dispatched to terminal/logs.\nPlease retrieve the PIN code and verify.`,
					);
				} else if (data.token) {
					...
					Alert.alert(
						...
						"JWT obtained directly. Session established.",
					);
					setShowSettings(false);
				}
			} else {
			...
			Alert.alert(
				"Auth Failed",
				`Credentials verification failed at BFF: ${err.message}`,
			);
		} finally {
	...
	const handleVerifyMfa = async () => {
		...
		try {
			...
			if (response.ok) {
				...
				Alert.alert(
					"MFA Authenticated",
					"Secure JWT session established. Ready for logistics dispatch.",
				);
				setShowSettings(false);
			} else {
				const errText = await response.text();
	...
	const injectDryIceCoolant = async () => {
		...
		if (useSimulator) {
			setTimeout(() => {
				setTemperature(4.0);
				addTelemetryLog(
					`[${new Date().toLocaleTimeString()}] ❄️ DRY ICE INJECTED: Coolant container active. Temp reset to 4.0°C.`,
					"info",
				);
				setCoolingLoading(false);
				setRiderStats((prev) => ({
					...prev,
					walletBalance: prev.walletBalance - 2.0,
				}));
				Alert.alert(
					"Coolant Injected",
					"Simulated dry ice cooling completed. Temperature reset to 4.0°C.",
				);
			}, 1000);
		} else {
			try {
				const response = await fetch(
					`${bffUrl}/api/telemetry/${orderId}/dry-ice`,
					{
						method: "POST",
						headers: {
							Authorization: `Bearer ${jwtToken}`,
				...
				if (response.ok) {
					...
					setTemperature(4.0);
					addTelemetryLog(
						`[${new Date().toLocaleTimeString()}] ❄️ DRY ICE INJECTED (BFF): Telemetry database updated. Temp reset to 4.0°C.`,
						"info",
					);
					setRiderStats((prev) => ({
						...prev,
					...
					Alert.alert(
						"Dry Ice Injected",
						"Dry ice cargo cooling completed. Telemetry database updated.",
					);
				} else {
					const errText = await response.text();
					throw new Error(errText || `HTTP ${response.status}`);
				...
				Alert.alert(
					"API Error",
					`Could not trigger coolant injection at BFF Gateway:\n${err.message}`,
				);
			} finally {
				setCoolingLoading(false);
			}
	...
	const triggerMockDispatch = () => {
		...
		setTelemetryLogs([]);
		addTelemetryLog(
			"New delivery dispatch proposal received from Zurich MFC.",
			"warning",
		);
	};
	...
	const handleAcceptDelivery = () => {
		if (useSimulator) {
			if (deliveryState === "idle") {
				...
				setTelemetryLogs([]);
				addTelemetryLog(
					"Rider accepted Order #4890. Initializing dispatch preparation.",
					"info",
				);
			} else {
				setDeliveryState("accepted");
				addTelemetryLog(
					`Rider accepted Order #${activeOrder.id}. Preparing E-Bike loading.`,
					"info",
				);
			}
		} else {
			...
			if (isNaN(orderId)) {
				Alert.alert(
					"Input Error",
					"Please enter a valid numeric Order ID from the queue.",
				);
				return;
			}
			...
			setTelemetryLogs([]);
			addTelemetryLog(
				`Rider manually assigned and accepted Order #${orderId}.`,
				"info",
			);
		}
	};
	...
	const handleDepartTransit = () => {
		...
		addTelemetryLog(
			"Transit departed. GPS route mapping started towards client geofence.",
			"info",
		);
	};
	...
	const handleCompleteDelivery = () => {
		...
		if (temperature >= 12.0) {
			Alert.alert(
				"Spoilage Alert",
				"Cannot complete delivery. Cargo temperature exceeded 12°C. Perishables are spoiled!",
			);
			return;
		}
		...
		const payout = isLightning ? 6.5 : 5.0;
	...
	const handleReportIncident = () => {
		...
		addTelemetryLog(
			`[${new Date().toLocaleTimeString()}] 🚨 EMERGENCY REPORTED: ${selectedIncidentType.toUpperCase()}`,
			"error",
		);
		if (incidentDetail) {
		...
		addTelemetryLog(
			"Operations Control: Dispatching backup E-Bike courier for cargo salvage.",
			"warning",
		);
		...
		setTimeout(() => {
			...
			addTelemetryLog(
				"Operations Control: Backup courier arrived at rider coordinates. Transferring cargo.",
				"info",
			);
			Alert.alert(
		...
		<SafeAreaView style={styles.container}>
			...
			<View style={styles.header}>
				<View style={styles.brandRow}>
					<Text style={styles.logoText}>
						SWISS <Text style={{ color: THEME.rider }}>Q-COMMERCE</Text>
					</Text>
				...
				<View style={styles.metricItem}>
					...
					<Text
						style={[
							...
							{
								color:
									riderStats.trustScore >= 85 ? THEME.customer : THEME.admin,
							},
						]}
					>
			...
			<ScrollView contentContainerStyle={styles.content}>
				...
				{!useSimulator && !isConnected && (
					<View style={styles.warningBanner}>
						...
						<Text style={styles.warningBannerText}>
							Real BFF mode active but disconnected! Check Settings.
						</Text>
					</View>
				)}
				...
				{backupRiderStatus === "dispatching" && (
					<View style={styles.incidentBanner}>
						...
						<Text style={styles.incidentBannerText}>
							EMERGENCY SALVAGE: Backup rider dispatching to coordinates...
						</Text>
					</View>
				)}
				...
				<View style={styles.pillInputRow}>
					<TouchableOpacity
						style={[
							styles.miniTab,
							activeTab === "navigation" && styles.miniTabActive,
						]}
					...
					>
						<Ionicons
							...
							size={14}
							color={
								activeTab === "navigation" ? "#070a13" : THEME.textSecondary
							}
						/>
						<Text
							style={[
								styles.miniTabText,
								activeTab === "navigation" && styles.miniTabTextActive,
							]}
						>
							Navigation
						</Text>
					...
					<TouchableOpacity
						style={[
							styles.miniTab,
							activeTab === "telemetry" && styles.miniTabActive,
						]}
					...
					>
						<Ionicons
							...
							size={14}
							color={
								activeTab === "telemetry" ? "#070a13" : THEME.textSecondary
							}
						/>
						<Text
							style={[
								styles.miniTabText,
								activeTab === "telemetry" && styles.miniTabTextActive,
							]}
						>
							IoT Logs
						</Text>
					...
					<TouchableOpacity
						style={[
							styles.miniTab,
							activeTab === "incidents" && styles.miniTabActive,
						]}
					...
					>
						<Ionicons
							...
							size={14}
							color={
								activeTab === "incidents" ? "#070a13" : THEME.textSecondary
							}
						/>
						<Text
							style={[
								styles.miniTabText,
								activeTab === "incidents" && styles.miniTabTextActive,
							]}
						>
							Emergency
						</Text>
					...
					<View style={styles.tabContent}>
							...
							<View style={styles.card}>
								<View style={styles.idleAnimationRow}>
									...
									<Text style={styles.idleDesc}>
										Standby at Central Dark Store MFC. Geofence active. Awaiting
										client dispatches...
									</Text>
								</View>
								...
								{useSimulator ? (
									...
									<View style={styles.manualAssignBox}>
										<Text style={styles.inputLabel}>
											Enter Order ID from Picker Queue:
										</Text>
										<TextInput
						...
						{deliveryState === "pending_accept" && activeOrder && (
							<View
								style={[
									styles.card,
									{ borderColor: THEME.rider, borderWidth: 1 },
								]}
							>
								<View style={styles.cardHeaderRow}>
									<Text style={styles.cardHeaderTitle}>
										🚨 INCOMING DELIVERY DISPATCH
									</Text>
								...
								<View style={styles.orderInfoBox}>
									...
									<Text style={styles.infoText}>
										{activeOrder.weather} (SLA Paused on Extreme Storms)
									</Text>
								</View>
						...
						{deliveryState === "accepted" && activeOrder && (
							<View style={styles.card}>
								...
								<Text style={styles.cardDescription}>
									Load cargo into the temperature-regulated thermal saddlebag.
									Verify cooling seal before departure.
								</Text>
							...
							<View style={styles.card}>
								<View style={styles.cardHeaderRow}>
									...
									<View style={styles.slaTimerBox}>
										<Ionicons
											...
											size={12}
											color={
												slaCountdown < 90 ? THEME.admin : THEME.textSecondary
											}
										/>
										<Text
											style={[
												styles.slaTimerText,
												slaCountdown < 90 && { color: THEME.admin },
											]}
										>
											SLA: {Math.floor(slaCountdown / 60)}:
											{String(slaCountdown % 60).padStart(2, "0")}
										</Text>
									</View>
								</View>
								...
								<View style={styles.progressSection}>
									...
									<Text style={styles.gpsCoordText}>
										GPS Node Coordinates: {gpsCoords.latitude},{" "}
										{gpsCoords.longitude}
									</Text>
								</View>
								...
								<View style={styles.coldChainTelemetryBlock}>
									<View style={styles.tempDisplayHeader}>
										<Text style={styles.tempLabel}>
											IoT COLD-CHAIN THERMOMETER
										</Text>
										{temperature > 8.0 && (
											<View style={styles.spoiledWarning}>
												<Text style={styles.spoiledWarningText}>
													⚠️ OVERHEATING
												</Text>
											</View>
										)}
									...
									<View style={styles.tempDigitalRow}>
										<Text
											style={[
												...
												temperature >= 6.0 &&
													temperature <= 8.0 && { color: THEME.rider },
									...
									<Text style={styles.coldChainDesc}>
										Warning: Perishables spoil rapidly above 8.0°C! Trigger dry
										ice coolant mitigation immediately if warning alerts
										trigger.
									</Text>
										...
										{coolingLoading ? (
											...
											<>
												<Ionicons
													...
													color="#070a13"
												/>
												<Text style={styles.coolantBtnText}>
													INJECT DRY ICE / COOLANT
												</Text>
											</>
										)}
									</TouchableOpacity>
								...
								<View style={styles.emergencyIncidentSection}>
									<TouchableOpacity
										style={styles.incidentTriggerBtn}
									...
									>
										<Ionicons
											...
											color={THEME.admin}
										/>
										<Text style={styles.incidentTriggerText}>
											REPORT INCIDENT / VEHICLE BREAKDOWN
										</Text>
									</TouchableOpacity>
								</View>
								...
								<TouchableOpacity
									style={[
										...
										routeProgress < 100 && { backgroundColor: THEME.textMuted },
									]}
							...
							<View style={styles.card}>
								<View style={styles.deliveredHeader}>
									...
									<Text style={styles.celebrationSub}>
										Order #{activeOrder.id} successfully completed. SLA
										verification matched client expectations.
									</Text>
								</View>
								...
								<View style={styles.telemetryGrid}>
									...
									<View style={styles.telemetryRow}>
										...
										<Text
											style={[styles.telemetryVal, { color: THEME.customer }]}
										>
											SLA Guarantee Verified
										</Text>
									</View>
									<View style={styles.telemetryRow}>
										<Text style={styles.telemetryLabel}>
											Base Commission Payout:
										</Text>
						...
						{deliveryState === "incident_blocked" && (
							<View
								style={[
									styles.card,
									{ borderColor: THEME.admin, borderWidth: 1 },
								]}
							>
								<View style={styles.deliveredHeader}>
									<View
										style={[
											styles.deliveredBadgeRing,
											{
									...
									<Text
										style={[styles.celebrationTitle, { color: THEME.admin }]}
									>
										TRANSIT BLOCKED
									</Text>
									<Text style={styles.celebrationSub}>
										Cold Chain breach or vehicle incident forced transit halt.
										Deliveries safety protocols activated.
									</Text>
								</View>
								...
								<View style={styles.telemetryGrid}>
									<View style={styles.telemetryRow}>
										...
										<Text style={[styles.telemetryVal, { color: THEME.admin }]}>
											Cargo Temperature Spoiled (12°C+)
										</Text>
									</View>
									<View style={styles.telemetryRow}>
										<Text style={styles.telemetryLabel}>
				...
				{activeTab === "telemetry" && (
					<View style={styles.tabContent}>
						<View style={styles.card}>
							<View style={styles.cardHeaderRow}>
								<Text style={styles.cardHeaderTitle}>
									IoT Telemetry Log Tracker
								</Text>
								<TouchableOpacity
									style={styles.clearLogsBtn}
									onPress={() => setTelemetryLogs([])}
								>
							...
							<Text style={styles.cardDescription}>
								Rolling logs from the E-Bike telemetry sensor node and BFF API
								server connections (refreshes on telemetry ticks).
							</Text>
							...
							>
								{telemetryLogs.length === 0 ? (
									<Text style={styles.consolePlaceholder}>
										[Telemetry log is empty. Start transit route to poll
										sensors]
									</Text>
								) : (
				...
				{activeTab === "incidents" && (
					<View style={styles.tabContent}>
						<View style={styles.card}>
							...
							<Text style={styles.cardDescription}>
								Use this dashboard to flag transit halts, vehicle breakdowns, or
								route accidents. Reporting incidents automatically invokes
								operations salvage protocols.
							</Text>
							...
							{deliveryState !== "in_transit" ? (
								<View style={styles.emptyContainer}>
									...
									<Text style={styles.emptySubText}>
										Incident console is only accessible during active transit
										dispatch.
									</Text>
								</View>
							) : (
									...
									<View style={styles.incidentTypeRow}>
										<TouchableOpacity
											style={[
												...
												selectedIncidentType === "breakdown" &&
													styles.incidentTypeBtnActive,
											]}
							...
							<View style={styles.settingsToggleRow}>
								<View style={{ flex: 1 }}>
									...
									<Text style={styles.toggleDesc}>
										When active, telemetry logs and dry-ice injections are
										mocked. Turn off for live BFF integration.
									</Text>
								</View>
								<Switch
									value={useSimulator}
								...
								{authStep === "login" ? (
									...
									<View style={styles.authForm}>
										<Text style={styles.mfaInstructionText}>
											Enter the 6-digit MFA verification PIN printed in the
											spring-boot backend terminal console.
										</Text>
										<TextInput
											style={[
												styles.textInput,
										...
										>
											{authLoading ? (
												...
												<Text
													style={[
														styles.authActionBtnText,
														{ color: "#070a13" },
													]}
												>
													VERIFY MFA & UNLOCK SESSION
												</Text>
											)}
										</TouchableOpacity>
										<TouchableOpacity
										...
										>
											<Text style={styles.authCancelBtnText}>
												Back to Credentials
											</Text>
										</TouchableOpacity>
									</View>
			...
			>
				<View style={styles.modalOverlay}>
					<View style={styles.settingsModalCard}>
						<View style={styles.modalHeader}>
							<Text style={[styles.settingsTitle, { color: THEME.admin }]}>
