...
const INITIAL_ORDERS = [
  {
    ...
    items: [
      { itemId: 'milk', name: 'Swiss Whole Milk 1L', quantity: 2, category: 'Dairy', emoji: '🥛', perishable: true },
      { itemId: 'bread', name: 'Artisan Sourdough Loaf', quantity: 1, category: 'Bakery', emoji: '🍞', perishable: false },
      { itemId: 'apples', name: 'Gala Apples 500g', quantity: 4, category: 'Produce', emoji: '🍎', perishable: false }
    ]
  },
  {
    orderId: 3012,
    ...
    items: [
      { itemId: 'chocolate', name: 'Lindt Dark Chocolate 70%', quantity: 3, category: 'Sweets', emoji: '🍫', perishable: false },
      { itemId: 'icecream', name: 'Movenpick Vanilla 500ml', quantity: 2, category: 'Frozen', emoji: '🍨', perishable: true }
    ]
  },
  {
    orderId: 3013,
    ...
    items: [
      { itemId: 'cola', name: 'Coca Cola Zero 6x330ml', quantity: 1, category: 'Beverages', emoji: '🥤', perishable: false },
      { itemId: 'chips', name: 'Zweifel Paprika Chips', quantity: 2, category: 'Snacks', emoji: '🥔', perishable: false }
    ]
  },
  {
    orderId: 3014,
    ...
    items: [
      { itemId: 'milk', name: 'Swiss Whole Milk 1L', quantity: 1, category: 'Dairy', emoji: '🥛', perishable: true },
      { itemId: 'eggs', name: 'Organic Free Range Eggs x12', quantity: 1, category: 'Dairy', emoji: '🥚', perishable: true },
      { itemId: 'cheese', name: 'Gruyère AOP aged 200g', quantity: 2, category: 'Dairy', emoji: '🧀', perishable: true }
    ]
  }
];
...
const DEFAULT_ITEMS = [
  ...
  { id: 'bread', name: 'Artisan Sourdough Loaf', emoji: '🍞', category: 'Bakery' },
  { id: 'apples', name: 'Gala Apples 500g', emoji: '🍎', category: 'Produce' },
  { id: 'chocolate', name: 'Lindt Dark Chocolate 70%', emoji: '🍫', category: 'Sweets' },
  { id: 'icecream', name: 'Movenpick Vanilla 500ml', emoji: '🍨', category: 'Frozen' },
  { id: 'cola', name: 'Coca Cola Zero 6x330ml', emoji: '🥤', category: 'Beverages' },
  { id: 'chips', name: 'Zweifel Paprika Chips', emoji: '🥔', category: 'Snacks' }
];
...
export default function PickerScreen() {
  ...
  const [jwtToken, setJwtToken] = useState('eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiJwaWNrZXJfZGVtb24iLCJyb2xlcyI6IlBJQ0tFUiIsImlhdCI6MTc4MDAwMDAwMH0.signature');
    ...
    const timer = setInterval(() => {
      ...
      if (useSimulator && autoDeployBackup && orders.length > 0) {
        ...
        if (Math.random() < 0.08) {
          setOrders(prev => {
            ...
            // Remove oldest order
            const next = [...prev];
            const sorted = next.sort((a,b) => a.slaCountdownSec - b.slaCountdownSec);
            ...
            setPickerStats(stats => ({
              ...
              avgPickTime: Math.max(45, Math.round((stats.avgPickTime * stats.pickedCount + (75 + Math.random()*20)) / (stats.pickedCount + 1)))
            }));
      ...
      if (useSimulator && Math.random() < 0.05 && orders.length < 8) {
        ...
        for (let i = 0; i < numItems; i++) {
          const rawItem = DEFAULT_ITEMS[Math.floor(Math.random() * DEFAULT_ITEMS.length)];
          const qty = 1 + Math.floor(Math.random() * 3);
          const price = 2.50 + Math.random() * 8.50;
          total += price * qty;
          orderItems.push({
            itemId: rawItem.id,
            name: rawItem.name,
  ...
  const testBffConnection = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        Alert.alert('BFF Connection Secure', 'BFF Gateway reached. Active JWT Session verified.');
      ...
      Alert.alert('Connection Failed', `Could not reach BFF Gateway at ${bffUrl}. Reverting to high-fidelity Offline Simulator.\n\nDetail: ${err.message}`);
    } finally {
      setLoading(false);
    }
  };
  ...
  const fetchBffQueue = async () => {
    ...
    try {
      const response = await fetch(`${bffUrl}/api/inventory/picker/queue?storeId=${selectedStore}`, {
        method: 'GET',
        headers: {
          'Authorization': `Bearer ${jwtToken}`,
          'Accept': 'application/json'
      ...
      if (response.ok) {
        ...
        const formattedOrders = data.map(o => {
          ...
          return {
            ...
            totalAmount: o.totalAmount || o.total_amount || 15.00,
  ...
  const completeHandover = async (containsError = false) => {
    ...
    const uncheckedCount = activePickingOrder.items.filter(item => !pickedItemChecks[item.itemId]).length;
    if (uncheckedCount > 0 && !containsError) {
      Alert.alert(
        ...
        [
          ...
          { text: 'Force Handover (Log error)', onPress: () => processHandover(true) }
        ]
      );
      return;
    }
  ...
  const processHandover = async (containsError) => {
    ...
    if (useSimulator) {
      ...
      setTimeout(() => {
        ...
        setOrders(prev => prev.filter(o => o.orderId !== activePickingOrder.orderId));
        ...
        setPickerStats(stats => ({
          ...stats,
          pickedCount: stats.pickedCount + 1,
          lightningBadges: awarded ? stats.lightningBadges + 1 : stats.lightningBadges,
          avgPickTime: Math.round((stats.avgPickTime * stats.pickedCount + duration) / (stats.pickedCount + 1)),
          trustScore: containsError ? Math.max(50, stats.trustScore - 8) : Math.min(100, stats.trustScore + 2),
        }));
        ...
        if (awarded) {
          ...
          Alert.alert('Cargo Handed Over', `Handover to rider ${selectedRider} logged in ${duration}s. Status set to SHIPPING.`);
        }
      }, 800);
    } else {
      // Integration with BFF Gateway API
      try {
        const response = await fetch(`${bffUrl}/api/inventory/picker/handover`, {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${jwtToken}`,
        ...
        if (response.ok) {
          ...
          const badgeAwarded = resData.lightningBadgeAwarded || resData.lightning_bonus_awarded;
          
          setPickerStats(stats => ({
            ...stats,
            pickedCount: stats.pickedCount + 1,
            lightningBadges: badgeAwarded ? stats.lightningBadges + 1 : stats.lightningBadges,
            avgPickTime: Math.round((stats.avgPickTime * stats.pickedCount + duration) / (stats.pickedCount + 1)),
            trustScore: containsError ? Math.max(50, stats.trustScore - 8) : Math.min(100, stats.trustScore + 1),
          }));
          ...
          if (badgeAwarded) {
            ...
            Alert.alert('Handover Success', `Payload delivered to BFF Gateway. Order routed to dispatch. duration=${duration}s.`);
          }
        } else {
        ...
        setHandoverLoading(false);
        Alert.alert('BFF Handover Error', `Failed to register handover at BFF: ${err.message}`);
      }
    }
  };
  ...
  const dispatchStockRequest = async () => {
    ...
    if (useSimulator) {
      ...
      setTimeout(() => {
        ...
        Alert.alert('Transfer Dispatched', `Autonomous truck ${result.transferTruckId} routing ${dispatchQty} units of ${result.item}.`);
      }, 1200);
    } else {
      try {
        ...
        if (response.ok) {
          ...
          setLastDispatchResult({
            ...
            fromStore: (resData.fromStore || dispatchSource).toUpperCase() + ' MFC',
            toStore: (resData.toStore || dispatchTarget).toUpperCase() + ' MFC',
            transferTruckId: resData.transferTruckId || resData.transfer_truck_id || ('TX-' + Math.floor(Math.random()*10000)),
            timestamp: new Date().toLocaleTimeString()
          });
          Alert.alert('BFF Gateway Success', 'Stock rebalance transaction committed to PostgreSQL Ledger.');
          fetchBffQueue();
        } else {
          const errorMsg = await response.text();
          throw new Error(errorMsg || `HTTP ${response.status}`);
        ...
        setDispatchLoading(false);
        Alert.alert('BFF Dispatch Error', `Failed to rebalance stock: ${err.message}`);
      }
    }
  };
  ...
  const executeVirtualScaling = () => {
    if (isOverflowBayDeployed) {
      Alert.alert('Max Scale Reached', 'Virtual overflow capacity limits are already active for this store zone.');
      return;
    }
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.header}>
        <View style={styles.brandRow}>
          <Text style={styles.logoText}>SWISS <Text style={{ color: THEME.inventory }}>Q-COMMERCE</Text></Text>
        ...
        <View style={styles.subHeader}>
          <View style={styles.roleContainer}>
            <MaterialCommunityIcons name="forklift" size={16} color={THEME.inventory} />
      ...
      <View style={styles.metricsContainer}>
        <View style={styles.metricItem}>
          ...
          <Text style={[styles.metricValue, { color: THEME.customer }]}>{pickerStats.pickedCount}</Text>
        </View>
        <View style={styles.metricItem}>
        ...
        <View style={styles.metricItem}>
          ...
          <Text style={[styles.metricValue, { color: THEME.engine }]}>{pickerStats.avgPickTime}s</Text>
        </View>
        <View style={styles.metricItem}>
          <Text style={styles.metricLabel}>TRUST SCORE</Text>
          <Text style={[styles.metricValue, { color: pickerStats.trustScore >= 80 ? THEME.customer : THEME.admin }]}>
      ...
      <View style={styles.storeSelectorContainer}>
        ...
        <View style={styles.storePillRow}>
          <TouchableOpacity 
            style={[styles.storePill, selectedStore === 'central' && styles.storePillActive]}
            onPress={() => setSelectedStore('central')}
          >
            <Text style={[styles.storePillText, selectedStore === 'central' && styles.storePillTextActive]}>
              Central MFC
            </Text>
          </TouchableOpacity>
          <TouchableOpacity 
            style={[styles.storePill, selectedStore === 'east' && styles.storePillActive]}
            onPress={() => setSelectedStore('east')}
          >
            <Text style={[styles.storePillText, selectedStore === 'east' && styles.storePillTextActive]}>
              East MFC
            </Text>
          </TouchableOpacity>
        </View>
      ...
      <ScrollView contentContainerStyle={styles.content}>
          ...
          <View style={styles.tabContent}>
            ...
            {filteredOrders.some(o => o.slaCountdownSec < 60) && (
              <View style={styles.slaAlertBanner}>
                <Ionicons name="warning-sharp" size={16} color="#070a13" style={{ marginRight: 8 }} />
                <Text style={styles.slaAlertText}>SLA BREACH CRITICAL: Pick order immediately!</Text>
              </View>
            )}
            ...
            <View style={styles.sectionHeaderRow}>
              ...
              {autoDeployBackup && (
                <View style={styles.backupRunningBadge}>
                  <Text style={styles.backupRunningText}>AUTO-PICKERS ACTIVE: {backupPickersActive}</Text>
                </View>
              )}
            </View>
            ...
            {filteredOrders.length === 0 ? (
              <View style={styles.emptyContainer}>
                <MaterialCommunityIcons name="check-decagram" size={48} color={THEME.customer} />
                <Text style={styles.emptyText}>Dark Store Queue Clear</Text>
                <Text style={styles.emptySubText}>All orders picked and handed to shipping riders.</Text>
              </View>
            ) : (
              filteredOrders.map(order => {
                const isCritical = order.slaCountdownSec < 60;
                return (
                  <View key={order.orderId} style={[styles.orderCard, isCritical && styles.orderCardCritical]}>
                    <View style={styles.orderCardHeader}>
                      <Text style={styles.orderIdText}>Order #{order.orderId}</Text>
                      
                      <View style={styles.slaContainer}>
                        <Ionicons name="time-outline" size={14} color={isCritical ? THEME.admin : THEME.textSecondary} />
                        <Text style={[styles.slaText, isCritical && styles.slaTextCritical]}>
                    ...
                    <View style={styles.orderMetadata}>
                      <Text style={styles.metaText}>
                        Total Amount: <Text style={styles.boldText}>CHF {order.totalAmount.toFixed(2)}</Text>
                      </Text>
                      <Text style={styles.metaText}>
                        Items to Pick: <Text style={styles.boldText}>{order.items.reduce((sum, i) => sum + i.quantity, 0)} units</Text>
                      </Text>
                    </View>
          ...
          <View style={styles.tabContent}>
            <View style={styles.card}>
              ...
              <Text style={styles.cardDescription}>
                Dispatch autonomous trucks to rebalance product items from surplus zones to target fulfillment centers (PostgreSQL serializable ledger sync).
              </Text>
              ...
              <View style={styles.pillInputRow}>
                <TouchableOpacity 
                  style={[styles.miniPill, dispatchSource === 'central' && styles.miniPillActive]}
                  onPress={() => {
                ...
                <TouchableOpacity 
                  style={[styles.miniPill, dispatchSource === 'east' && styles.miniPillActive]}
                  onPress={() => {
              ...
              <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.itemPillScroller}>
                {DEFAULT_ITEMS.map(item => (
                  <TouchableOpacity
                    key={item.id}
                    style={[styles.itemSelectPill, dispatchItem === item.id && styles.itemSelectPillActive]}
              ...
              {/* Quantity */}
              <Text style={styles.inputLabel}>Transfer Quantity: {dispatchQty} units</Text>
              ...
              {/* Submit Dispatch */}
              <TouchableOpacity 
                style={[styles.dispatchSubmitBtn, dispatchLoading && { opacity: 0.7 }]} 
              ...
              >
                {dispatchLoading ? (
                  ...
                  <>
                    <Text style={styles.dispatchSubmitBtnText}>DISPATCH TRANSFER VEHICLE</Text>
            ...
            {lastDispatchResult && (
              <View style={[styles.card, { borderColor: THEME.customer, borderWidth: 1 }]}>
                <View style={styles.resultHeader}>
                  <Text style={[styles.resultTitle, { color: THEME.customer }]}>STOCK DISPATCH LOGGED</Text>
                  <Text style={styles.resultTime}>{lastDispatchResult.timestamp}</Text>
                </View>
                <View style={styles.resultGrid}>
                  <View style={styles.resultRow}>
                    <Text style={styles.resultLabel}>Carrier:</Text>
                    <Text style={styles.resultVal}>{lastDispatchResult.transferTruckId}</Text>
                  </View>
                  <View style={styles.resultRow}>
                  ...
                  <View style={styles.resultRow}>
                    ...
                    <Text style={styles.resultVal}>{lastDispatchResult.quantity} units</Text>
                  </View>
                  <View style={styles.resultRow}>
                    <Text style={styles.resultLabel}>Route Path:</Text>
                    <Text style={styles.resultVal}>{lastDispatchResult.fromStore} ➡️ {lastDispatchResult.toStore}</Text>
                  </View>
                </View>
              </View>
            )}
        ...
        {activeTab === 'ops' && (
          <View style={styles.tabContent}>
            ...
            <View style={styles.card}>
              <Text style={styles.cardTitle}>Dark Store Capacity & Virtual Scaling</Text>
              <Text style={styles.cardDescription}>
                Monitor physical storage occupancy. Deploy manual-scale virtual overflow warehouse bays to avoid order checkouts rejecting due to congestion.
              </Text>
              ...
              <View style={styles.occupancyGaugeContainer}>
                <View style={styles.occupancyMetricBox}>
                  ...
                  <Text style={[
                    ...
                    { color: storeCapacity >= 90 ? THEME.admin : storeCapacity >= 75 ? THEME.rider : THEME.customer }
                  ]}>
                    {storeCapacity}%
                  </Text>
                  <Text style={styles.occupancySub}>
                    {storeCapacity >= 90 ? 'SLA CHECKOUT BLOCK RISK' : 'OPERATIONAL CAPACITY OK'}
                  </Text>
                </View>
                ...
                <View style={styles.progressTrackBar}>
                  <View style={[
                    ...
                    { 
                      width: `${storeCapacity}%`,
                      backgroundColor: storeCapacity >= 90 ? THEME.admin : storeCapacity >= 75 ? THEME.rider : THEME.inventory 
                    }
                  ]} />
                </View>
              </View>
              ...
              {isOverflowBayDeployed ? (
                <View style={styles.overflowSuccessCard}>
                  <Ionicons name="checkmark-circle" size={18} color="#070a13" style={{ marginRight: 8 }} />
                  <View style={{ flex: 1 }}>
                    <Text style={styles.overflowSuccessTitle}>VIRTUAL BAY ACTIVE</Text>
                    <Text style={styles.overflowSuccessSub}>+300 sq meters added. Congestion mitigated.</Text>
                  </View>
                  <TouchableOpacity style={styles.resetScaleBtn} onPress={() => {
                ...
                >
                  {isScaling ? (
                    <View style={styles.scalingIndicatorRow}>
                      ...
                      <Text style={styles.scaleActionBtnText}>SCALING MATRIX: {scalingProgress}%</Text>
                    </View>
                  ) : (
                    <>
                      <Text style={styles.scaleActionBtnText}>DEPLOY VIRTUAL OVERFLOW BAY</Text>
                      <MaterialCommunityIcons name="arrow-expand-all" size={16} color="#070a13" />
                    </>
                  )}
                </TouchableOpacity>
              )}
            ...
            <View style={styles.card}>
              <View style={styles.cardHeaderRow}>
                <View style={{ flex: 1 }}>
                  ...
                  <Text style={styles.cardDescription}>
                    Trigger backup picker daemons automatically when order backlog rises above 4 pending tickets to maintain 4-minute SLA.
                  </Text>
                </View>
                <Switch
              ...
              <View style={styles.rosterStatusContainer}>
                ...
                <View style={styles.rosterStatusRow}>
                  ...
                  <View style={styles.rosterStatItem}>
                    <Text style={[styles.rosterStatNum, { color: autoDeployBackup ? THEME.customer : THEME.textMuted }]}>
                      {backupPickersActive}
                    </Text>
                    <Text style={styles.rosterStatLabel}>Backup Deployed</Text>
                  </View>
                  <View style={styles.rosterStatItem}>
                    <Text style={styles.rosterStatNum}>{1 + backupPickersActive}</Text>
                ...
                {autoDeployBackup && backupPickersActive > 0 && (
                  <View style={styles.deployPulseAnimationContainer}>
                    ...
                    <Text style={styles.pulseText}>Backup pickers actively draining queue bottlenecks</Text>
                  </View>
                )}
              </View>
            </View>
            ...
            <View style={styles.card}>
              ...
              <View style={styles.telemetryGrid}>
                <View style={styles.telemetryRow}>
                  ...
                  <Text style={[styles.telemetryVal, { color: isConnected ? THEME.customer : THEME.rider }]}>
                    {isConnected ? 'SECURE CONNECTION' : 'OFFLINE SIMULATED'}
                  </Text>
                </View>
                <View style={styles.telemetryRow}>
                  <Text style={styles.telemetryLabel}>Ledger Integrity:</Text>
                  <Text style={[styles.telemetryVal, { color: THEME.customer }]}>VERIFIED (HASH-CHAIN OK)</Text>
                </View>
                <View style={styles.telemetryRow}>
                  <Text style={styles.telemetryLabel}>Double-Entry Audit:</Text>
                  <Text style={[styles.telemetryVal, { color: THEME.customer }]}>DEBITS == CREDITS INVARIANT MET</Text>
                </View>
                <View style={styles.telemetryRow}>
                  <Text style={styles.telemetryLabel}>Database Latency:</Text>
                  <Text style={[styles.telemetryVal, { fontFamily: 'monospace' }]}>4.2 ms (OLTP)</Text>
                </View>
              </View>
            </View>
          </View>
          ...
          <View style={styles.modalOverlay}>
            <View style={styles.pickModalContainer}>
              <View style={styles.pickModalHeader}>
                <View>
                  <Text style={styles.pickModalTitle}>Picking Order #{activePickingOrder.orderId}</Text>
                  <Text style={styles.pickModalSub}>Store Location: {activePickingOrder.storeId.toUpperCase()} MFC</Text>
                </View>
                <TouchableOpacity 
              ...
              <ScrollView style={styles.checklistScroll}>
                {activePickingOrder.items.map((item, idx) => {
                  ...
                  return (
                    <TouchableOpacity 
                      key={idx} 
                      style={[styles.checkItemRow, isChecked && styles.checkItemRowChecked]}
                      onPress={() => toggleItemCheck(item.itemId)}
                    >
                      <View style={[styles.checkbox, isChecked && styles.checkboxChecked]}>
                        {isChecked && <Ionicons name="checkmark" size={14} color="#070a13" />}
                      </View>
                      <View style={{ flex: 1 }}>
                        <Text style={[styles.itemNameText, isChecked && styles.itemNameTextChecked]}>
                          {item.name}
                        </Text>
                        <View style={styles.itemTagRow}>
                          ...
                          {item.perishable && (
                            <View style={styles.perishableBadge}>
                              <Text style={styles.perishableBadgeText}>❄️ PERISHABLE</Text>
                            </View>
                          )}
                        </View>
                      </View>
              ...
              <View style={styles.handoverFormContainer}>
                ...
                <View style={styles.handoverBtnRow}>
                  <TouchableOpacity 
                    style={[styles.handoverBtn, styles.handoverBtnError, handoverLoading && { opacity: 0.6 }]}
                  ...
                  <TouchableOpacity 
                    style={[styles.handoverBtn, styles.handoverBtnSuccess, handoverLoading && { opacity: 0.6 }]}
                  ...
                  >
                    {handoverLoading ? (
                      ...
                      <>
                        ...
                        <Ionicons name="arrow-forward-sharp" size={16} color="#070a13" />
                      </>
                    )}
                  </TouchableOpacity>
                </View>
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.settingsModalCard}>
            ...
            <ScrollView contentContainerStyle={styles.settingsScrollContent}>
              <Text style={styles.cardDescription}>
                Connect to the Live Spring Cloud Gateway BFF and complete transactions in PostgreSQL via dual-ledger entry verification.
              </Text>

              {/* Simulator Mode toggle */}
              <View style={styles.settingsToggleRow}>
                <View style={{ flex: 1 }}>
                  <Text style={styles.toggleLabel}>Enable Offline Simulator Mode</Text>
                  <Text style={styles.toggleDesc}>
                    Run high-fidelity client-side database simulation without needing the active BFF server running.
                  </Text>
                </View>
                <Switch
                  value={useSimulator}
              ...
              {!useSimulator && (
                <View style={styles.bffConfigArea}>
                  ...
                  <Text style={styles.inputLabel}>Bearer JWT Session Authorization Token:</Text>
                  <TextInput
                  ...
                  <TouchableOpacity style={styles.testBtn} onPress={testBffConnection}>
                    {loading ? (
                      <ActivityIndicator size="small" color="#070a13" />
                    ) : (
                      <>
                        <Text style={styles.testBtnText}>VALIDATE BFF GATEWAY</Text>
                        <Ionicons name="shield-checkmark" size={16} color="#070a13" />
                      </>
                    )}
                  </TouchableOpacity>
                </View>
      ...
      <Modal visible={lightningCelebration} transparent={true} animationType="fade">
        <View style={styles.modalOverlay}>
          <View style={styles.celebrationCard}>
            <Text style={styles.celebrationFlash}>⚡ LIGHTNING PICKER ⚡</Text>
            <View style={styles.badgeRing}>
              <MaterialCommunityIcons name="lightning-bolt" size={80} color="#fbbf24" />
            </View>
            <Text style={styles.celebrationTitle}>SLA SPEED BADGE AWARDED</Text>
            <Text style={styles.celebrationSub}>
              Order pick handover completed in under 90 seconds. Your speed record was committed to the Security Trust Ledger.
            </Text>
            <TouchableOpacity 
      ...
      <View style={styles.tabBar}>
        <TouchableOpacity 
          style={[styles.tabBarItem, activeTab === 'queue' && styles.tabBarItemActive]}
        ...
        >
          ...
          />
          <Text style={[styles.tabBarText, activeTab === 'queue' && styles.tabBarTextActive]}>
            PICK QUEUE
          </Text>
        </TouchableOpacity>

        <TouchableOpacity 
          style={[styles.tabBarItem, activeTab === 'dispatch' && styles.tabBarItemActive]}
          onPress={() => setActiveTab('dispatch')}
        >
          <FontAwesome5 
            name="truck" 
            size={18} 
            color={activeTab === 'dispatch' ? THEME.inventory : THEME.textSecondary} 
          />
          <Text style={[styles.tabBarText, activeTab === 'dispatch' && styles.tabBarTextActive]}>
            STOCK DISPATCH
          </Text>
        </TouchableOpacity>

        <TouchableOpacity 
          style={[styles.tabBarItem, activeTab === 'ops' && styles.tabBarItemActive]}
        ...
        >
          ...
          />
          <Text style={[styles.tabBarText, activeTab === 'ops' && styles.tabBarTextActive]}>
            OPERATIONS
          </Text>
        </TouchableOpacity>
      </View>
